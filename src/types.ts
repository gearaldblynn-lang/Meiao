...
export const TranslationSubModeObj = {
  MAIN: TranslationSubMode.MAIN,
  DETAIL: TranslationSubMode.DETAIL,
  REMOVE_TEXT: TranslationSubMode.REMOVE_TEXT,
} as const;
...
export interface WorkspacePreferences {
  ...
  playSoundAfterGeneration: boolean;
...
export interface VideoStoryboardConfig {
  ...
  aspectRatio: AspectRatio.SQUARE | AspectRatio.P_3_4 | AspectRatio.L_4_3 | AspectRatio.P_9_16 | AspectRatio.L_16_9 | AspectRatio.L_21_9;
...
export interface VideoStoryboardProject {
  ...
  status: 'pending' | 'scripting' | 'awaiting_image_confirmation' | 'imaging' | 'completed' | 'failed';
  script: string;
...
export interface OneClickMaterialSnapshot {
  id: string;
  type: string;
  url: string;
  remoteUrl?: string;
...
export interface XhsCoverPersistentState {
  ...
  uploadedStyleReferenceUrl?: string | null;
  title: string;
  subtitle: string;
