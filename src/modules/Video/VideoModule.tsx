...
import { useToast } from '../../shell/components/ToastSystem';
...
const VideoModule: React.FC<Props> = ({ apiConfig, persistentState, onStateChange }) => {
  ...
  const shouldGenerateWhiteBgForProject = (config: VideoStoryboardConfig, isFirstProject = true) =>
    config.videoGenerationMode === 'viral_split' ? false : (config.generateWhiteBg && isFirstProject);
  ...
  const runProjectImaging = async (
    ...
    updateProject(projectId, (project) => ({
      ...
      status: 'imaging',
      error: undefined,
      whiteBgStatus: includeWhiteBg ? (project.whiteBgStatus || 'pending') : project.whiteBgStatus,
    }));
  ...
  const processProject = async (
    ...
    try {
      ...
      updateProject(projectId, (project) => ({
        ...
        shots,
        boards,
        status: config.videoGenerationMode === 'viral_split' ? 'awaiting_image_confirmation' : 'imaging',
      ...
      await runProjectImaging(projectId, config, shots, boards, includeWhiteBg);
      void logActionSuccess({
        module: 'video',
        action: 'generate_storyboard_project',
        message: '短视频分镜项目生成完成',
  ...
  const handleConfirmProjectImaging = async (projectId: string) => {
    const project = storyboard.projects.find((item) => item.id === projectId);
    if (!project) return;
    if (project.status !== 'awaiting_image_confirmation') return;
    if (project.boards.length === 0) {
      addToast('还没有可用于生图的分镜方案', 'warning');
      return;
    }

    void logActionStart({
      module: 'video',
      action: 'confirm_storyboard_imaging',
      message: '确认开始分镜板生图',
      meta: {
        ...storyboardMeta,
        projectId,
    ...
    try {
      ...
      await runProjectImaging(project.id, project.config, project.shots, project.boards, shouldGenerateWhiteBgForProject(project.config));
      void logActionSuccess({
        module: 'video',
        action: 'confirm_storyboard_imaging',
        message: '确认生图任务完成',
      ...
      updateProject(projectId, (current) => ({ ...current, status: 'failed', error: error.message || '生图失败' }));
    ...
    try {
      ...
      for (let index = 0; index < nextProjects.length; index++) {
        await processProject(
          ...
          shouldGenerateWhiteBgForProject(runtimeConfig, index === 0)
        );
      }

      addToast(
        runtimeConfig.videoGenerationMode === 'viral_split'
          ? `已生成 ${nextProjects.length} 个爆款复刻策划，请确认后生图`
          : `已完成 ${nextProjects.length} 个视频方案`,
        'success'
      );
      void logActionSuccess({
        module: 'video',
  ...
  const handleRetryProject = async (projectId: string) => {
    ...
    await processProject(projectId, project.config, project.sceneDescription || '', shouldGenerateWhiteBgForProject(project.config));
  };
      ...
      {subMode === VideoSubMode.DIAGNOSIS ? (
        ...
        <>
          ...
          <StoryboardWorkspace
            ...
            onConfirmProjectImaging={handleConfirmProjectImaging}
