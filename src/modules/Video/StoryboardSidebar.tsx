...
const MAX_SHOT_COUNT: Record<string, number> = {
  '5s': 3,
  '10s': 6,
  '15s': 12,
  '30s': 12,
...
const StoryboardSidebar: React.FC<Props> = ({ config, disabled, subMode, onSubModeChange, onChange, onGenerate }) => {
  ...
  const maxShots = MAX_SHOT_COUNT[config.duration] ?? 12;
  ...
  const shotCountOptions = allShotOptions.filter((o) => o.value <= maxShots);
  ...
  const handleFieldChange = (
    ...
    onChange((prev) => {
      if (name === 'duration') {
        ...
        const max = MAX_SHOT_COUNT[nextDuration] ?? 12;
