...
const SkuSidebar: React.FC<Props> = ({
  ...
  const [countDraft, setCountDraft] = useState(String(config.count || 4));
  ...
  useEffect(() => {
    setCountDraft(String(config.count || 4));
  }, [config.count]);

  useEffect(() => {
    setTargetWidthDraft(config.targetWidth ? String(config.targetWidth) : '');
  }, [config.targetWidth, config.resolutionMode]);
  ...
  const commitCountDraft = () => {
    const next = Math.max(1, Math.min(20, Number(countDraft.replace(/[^\d]/g, '')) || 1));
    handleCountChange(String(next));
    setCountDraft(String(next));
  };

  const commitTargetWidthDraft = () => {
    const next = Number(targetWidthDraft.replace(/[^\d]/g, '')) || 0;
    onUpdateConfig((prev) => ({ ...prev, targetWidth: next, targetHeight: 0 }));
          ...
          <div className="px-4 pb-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter') commitCountDraft(); if (e.key === 'Escape') setCountDraft(String(config.count || 4)); }}
                disabled={disabled}
