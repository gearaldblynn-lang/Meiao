...
test('normalizeCopyLayoutLine rewrites legacy bullet copy rows into the strict template and preserves font names', () => {
  const line = '•主文案：「冷冻后更好吃」— 钉钉进步体，大，上方居中，深棕色';

  assert.equal(
    normalizeCopyLayoutLine(line),
    '主文案(钉钉进步体, 大, 上方居中, 深棕色):“冷冻后更好吃”'
  );
});

test('normalizeCopyLayoutLine preserves standard rows while normalizing quotes and keeping font tokens', () => {
  const line = '卖点文案(阿里巴巴普惠体, 中, 底部, 浅金色):"纯脂黑巧，微苦醇香不甜腻"';

  assert.equal(
    normalizeCopyLayoutLine(line),
    '卖点文案(阿里巴巴普惠体, 中, 底部, 浅金色):“纯脂黑巧，微苦醇香不甜腻”'
  );
});
...
文案内容排版：
主文案(钉钉进步体, 大, 上方居中, 深棕色):“冷冻后更好吃”
副文案(阿里巴巴普惠体, 中, 浅蓝色):“常温像慕斯，冷冻像冰淇淋”`
  );
});
