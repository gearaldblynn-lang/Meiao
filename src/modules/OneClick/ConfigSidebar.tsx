...
const ConfigSidebar: React.FC<Props> = ({
  ...
  const [countDraft, setCountDraft] = useState(String(config.count || 1));
  const [targetWidthDraft, setTargetWidthDraft] = useState(config.targetWidth ? String(config.targetWidth) : '');
  ...
  useEffect(() => {
    setCountDraft(String(config.count || 1));
  }, [config.count, subMode]);

  useEffect(() => {
    setTargetWidthDraft(config.targetWidth ? String(config.targetWidth) : '');
  }, [config.targetWidth, config.resolutionMode, subMode]);

  const [isCustomLanguage, setIsCustomLanguage] = useState(false);

  const commitCountDraft = () => {
    const next = Math.max(1, Math.min(15, Number(countDraft.replace(/[^\d]/g, '')) || 1));
    onChange({ ...config, count: next });
    setCountDraft(String(next));
  };

  const commitTargetWidthDraft = () => {
    const next = Number(targetWidthDraft.replace(/[^\d]/g, '')) || 0;
    onChange({ ...config, targetWidth: next, targetHeight: 0 });
    setTargetWidthDraft(next > 0 ? String(next) : '');
  };
  ...
  useEffect(() => {
    const defaultW = isDetail ? 750 : 800;
    const defaultH = isDetail ? 0 : 800;
    const defaultRatio = isDetail ? AspectRatio.P_3_4 : AspectRatio.SQUARE;
  ...
  const fallbackAspectRatio = isDetail ? AspectRatio.P_3_4 : AspectRatio.SQUARE;
              ...
              <div className="px-4 pb-4 space-y-4">
                ...
                <div className="grid gap-3 grid-cols-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <PopoverSelect
                      value={(config.aspectRatio || fallbackAspectRatio) as AspectRatio}
                      ...
                      buttonClassName="h-10 rounded-2xl px-4 text-xs"
                    />
                    <p className="ml-1 text-[10px]" style={{ color: 'var(--text-tertiary)' }}>
                      {isDetail ? '推荐：auto，默认：3:4' : '推荐：1:1'}
                    </p>
                  </div>
                  {isFirstImage ? (
                    <div className="space-y-1">
