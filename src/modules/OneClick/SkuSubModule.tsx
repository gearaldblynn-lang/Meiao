                  ...
                  <div className={`flex-1 p-6 flex items-center justify-center relative transition-opacity duration-300 ${scheme.selected ? 'bg-slate-50 opacity-100' : 'bg-slate-100/50 opacity-40'}`}>
                    {scheme.status === 'generating' ? (
                      <div className="text-center">
                        ...
                        <button onClick={() => handleInterruptSingle(scheme.id)} className="mt-4 rounded-lg bg-slate-200 px-4 py-2 text-xs font-medium text-slate-600 transition-all hover:bg-slate-300">中断</button>
                      </div>
          ...
          return (
            <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center" onClick={() => setPreviewId(null)}>
              ...
              <div className="absolute top-6 right-6 flex items-center gap-3">
                ...
                <button onClick={(e) => { e.stopPropagation(); void downloadRemoteFile(previewScheme.resultUrl!, `sku_${currentPreviewIndex + 1}.png`); }} className="w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-all" title="下载"><Download size={18} /></button>
