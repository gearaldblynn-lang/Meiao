...
const StatusBadge: React.FC<{ status: FolderCard['files'][number]['status'] }> = ({ status }) => {
  ...
  if (status === 'uploading') return (
    <span className="inline-flex items-center gap-1 rounded-full bg-cyan-50 px-2 py-0.5 text-[10px] font-bold text-cyan-600">
      <LegacyFaIcon icon="fa-spinner" className="animate-spin text-[9px]" />上传中
    </span>
  );
  ...
  return (
    <div className="mt-2 rounded-[16px] border border-slate-200/85 bg-white/96 shadow-[0_4px_14px_rgba(15,23,42,0.06)]">
      ...
      <div className="flex items-center gap-2.5 px-3 py-2.5">
        <LegacyFaIcon icon="fa-folder-open" className={`text-[14px] ${card.phase === 'done' ? 'text-emerald-500' : card.phase === 'error' ? 'text-rose-400' : 'text-amber-400'}`} />
        ...
        >
          <LegacyFaIcon icon={`fa-chevron-${card.expanded ? 'up' : 'down'}`} className="text-[10px]" />
        </button>
        <button
          type="button"
          onClick={onDismiss}
          ...
          title="取消并移除"
        >
          <LegacyFaIcon icon="fa-xmark" className="text-[11px]" />
        </button>
      </div>
        ...
        <div className="max-h-[180px] overflow-y-auto border-t border-slate-100 px-3 py-2">
          {card.files.map((f, i) => (
            <div key={`${f.relativePath}-${i}`} className="flex items-center gap-2 py-1">
              <LegacyFaIcon icon={getFileIcon(f.relativePath)} className="w-4 shrink-0 text-center text-[11px] text-slate-400" />
