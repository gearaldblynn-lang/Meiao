...
const getProgressStageLabel = (message: AgentChatMessage) => {
  ...
  if (stage === 'image_validating') return '正在生成图片';
  if (stage === 'image_validation_failed') return '正在生成图片';
  if (stage === 'image_regenerating') return '正在生成图片';
...
const getAssistantRunStages = (message: AgentChatMessage) => {
  ...
  if (metadata.requestMode === 'image_generation' || ['image_generating', 'image_validating', 'image_validation_failed', 'image_regenerating', 'image_ready'].includes(progressStage)) {
    pushStage({ key: 'image_generating', label: '生成图片', active: ['generating', 'image_generating', 'image_validating', 'image_validation_failed', 'image_regenerating'].includes(progressStage), done: !isPending || progressStage === 'image_ready' });
  }
