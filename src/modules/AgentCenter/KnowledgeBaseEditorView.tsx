...
const modalSurfaceClassName =
  'w-full rounded-[26px] border px-5 py-5 shadow-[0_22px_56px_rgba(15,23,42,0.16)]';

const modalSectionClassName = 'rounded-[18px] border px-4 py-4';
...
const KnowledgeBaseEditorView: React.FC<Props> = ({
  ...
  return (
    <>
      <div className="mx-auto w-full max-w-[1280px] space-y-4 pb-6 2xl:max-w-[1320px]">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  title="知识库使用说明"
                >
                  <LegacyFaIcon icon="fa-circle-question" className="text-[12px]" />
                </button>
              </div>
          ...
          <WorkspaceShellCard className="bg-white px-4 py-4 shadow-[0_18px_44px_rgba(148,163,184,0.08)]">
            ...
            <div className="mt-4 space-y-3">
              ...
              <PopoverSelect
                ...
                onChange={(value) => onDocumentFormChange('sourceType', value)}
                options={[
                  { value: 'manual', label: '手动录入' },
                  { value: 'upload', label: '文本上传' },
                ]}
                buttonClassName="h-[50px] rounded-[18px] px-4 text-[14px] font-medium"
              />
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={() => setShowChunkGuide(true)} className="flex h-5 w-5 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500" aria-label="查看切片策略说明" title="查看切片策略说明">
                    <LegacyFaIcon icon="fa-circle-question" className="text-[10px]" />
                  </button>
                </div>
                <PopoverSelect
                  value={documentForm.chunkStrategy}
                  onChange={(value) => onDocumentFormChange('chunkStrategy', value)}
                  options={Object.entries(KNOWLEDGE_CHUNK_STRATEGY_META).map(([key, meta]) => ({
                    value: key as 'general' | 'rule' | 'sop' | 'faq' | 'case',
                    label: meta.label,
                  }))}
                  buttonClassName="h-[50px] rounded-[18px] px-4 text-[14px] font-medium"
                />
      ...
      {showGuide ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/24 px-6">
          <div
            ...
            style={{ maxWidth: '48rem', background: 'var(--bg-surface)', borderColor: 'var(--border-subtle)' }}
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-[22px] font-black" style={{ color: 'var(--text-primary)' }}>知识库使用说明</h3>
                <p className="mt-1 text-[13px] font-medium" style={{ color: 'var(--text-secondary)' }}>给第一次配置知识库的同事快速上手。</p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <section className={modalSectionClassName} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>怎么配置</h4>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>先建知识库名称和部门，再新增文档。文档入库后，再回到智能体里绑定这个知识库。</p>
              </section>
              <section className={modalSectionClassName} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>原理是什么</h4>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>知识库不会整份塞给模型，而是先存文档，再按问题检索相关片段，把少量命中内容带给模型回答。</p>
              </section>
              <section className={modalSectionClassName} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>什么是切片</h4>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>切片就是把长文档拆成多个短片段，方便后续检索和引用。片段越清晰，命中越稳定。</p>
              </section>
              <section className={modalSectionClassName} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>支持格式</h4>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>当前支持 `.txt`、`.md`、`.csv`、`.json`，也支持直接手动粘贴文本。</p>
              </section>
              <section className={`${modalSectionClassName} md:col-span-2`} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>文档标准建议</h4>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>一份文档尽量只讲一类主题。标题清楚、段落短、规则明确，避免把多个部门或多个流程混在同一份文档里。</p>
              </section>
            </div>
          </div>
        </div>
      ...
      {showChunkGuide ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/24 px-6">
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-[22px] font-black" style={{ color: 'var(--text-primary)' }}>切片策略说明</h3>
                <p className="mt-1 text-[13px] font-medium" style={{ color: 'var(--text-secondary)' }}>不同内容类型，适合不同的切片方式。</p>
              </div>
              <button type="button" onClick={() => setShowChunkGuide(false)} className="flex h-9 w-9 items-center justify-center rounded-full border" style={{ background: 'var(--bg-input)', borderColor: 'var(--border-subtle)', color: 'var(--text-secondary)' }} aria-label="关闭切片策略说明">
                <LegacyFaIcon icon="fa-xmark" className="text-sm" />
              </button>
            </div>
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              {Object.entries(KNOWLEDGE_CHUNK_STRATEGY_META).map(([key, meta]) => (
                <section key={key} className={modalSectionClassName} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                  <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>{meta.label}</h4>
                  <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>{meta.description}</p>
                  <p className="mt-2 text-[12px] font-medium" style={{ color: 'var(--text-tertiary)' }}>默认单片长度上限：{meta.maxChunkChars} 字</p>
                </section>
              ))}
            </div>
          </div>
      ...
      {showNormalizationGuide ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/24 px-6">
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-[22px] font-black" style={{ color: 'var(--text-primary)' }}>AI 规范整理说明</h3>
                <p className="mt-1 text-[13px] font-medium" style={{ color: 'var(--text-secondary)' }}>用于把杂乱规则整理成更适合检索的结构化表达。</p>
              </div>
              <button type="button" onClick={() => setShowNormalizationGuide(false)} className="flex h-9 w-9 items-center justify-center rounded-full border" style={{ background: 'var(--bg-input)', borderColor: 'var(--border-subtle)', color: 'var(--text-secondary)' }} aria-label="关闭 AI 规范整理说明">
                <LegacyFaIcon icon="fa-xmark" className="text-sm" />
              </button>
            </div>
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <section className={`${modalSectionClassName} md:col-span-2`} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>什么是 AI 规范整理</h4>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>系统会先保留原文，再用大模型把杂乱规则整理成更适合检索的结构化表达，然后优先对整理版切片。整理失败时，会自动回退为原文切片，不会阻止入库。</p>
              </section>
              <section className={modalSectionClassName} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>什么时候建议开启</h4>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>适合规则文档、SOP、参数规范、提示词规范、决策规则库。尤其适合你这种生图规则、SKU 规则、多图输入规则。</p>
              </section>
              <section className={modalSectionClassName} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>什么时候不建议开启</h4>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>不适合案例库、原始素材记录、需要逐字保留原文的文档，也不适合内容本身已经结构很清晰的短文档。</p>
              </section>
              <section className={modalSectionClassName} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>优点</h4>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>命中更稳、规则更完整、对“检索参考数量”更友好，尤其能减少规则被切得过碎导致只命中半条规则的问题。</p>
              </section>
              <section className={modalSectionClassName} style={{ background: 'var(--bg-base)', borderColor: 'var(--border-subtle)' }}>
                <h4 className="text-[15px] font-black" style={{ color: 'var(--text-primary)' }}>风险</h4>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>模型可能改变原文表达，所以系统始终保留原文；若文档强调逐字准确，请关闭该开关，直接按原文切片。</p>
              </section>
            </div>
          </div>
        </div>
      ...
      {showDeleteConfirm ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/24 px-6">
          <div className={modalSurfaceClassName} style={{ maxWidth: '28rem', background: 'var(--bg-surface)', borderColor: 'var(--border-subtle)' }}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-[20px] font-black" style={{ color: 'var(--text-primary)' }}>永久删除确认</h3>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>确认删除后不可恢复，知识库、已入库文档和切片都会被一并删除。</p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => setShowDeleteConfirm(false)}
                className="rounded-[16px] border px-4 py-2 text-[13px] font-black"
                style={{ background: 'var(--bg-input)', borderColor: 'var(--border-subtle)', color: 'var(--text-secondary)' }}
              >
                取消
              </button>
              <button
                ...
                className="rounded-[16px] px-4 py-2 text-[13px] font-black text-white"
      ...
      {showDeleteDocumentConfirm ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/24 px-6">
          <div className={modalSurfaceClassName} style={{ maxWidth: '28rem', background: 'var(--bg-surface)', borderColor: 'var(--border-subtle)' }}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-[20px] font-black" style={{ color: 'var(--text-primary)' }}>确认删除文档</h3>
                <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--text-secondary)' }}>
