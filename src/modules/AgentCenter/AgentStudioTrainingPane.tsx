...
type StudioTrainingHistory = Array<{
  role: string;
  content: string;
    ...
    <div className="mt-2 flex flex-wrap gap-2">
      {attachments.map((attachment, index) => (
        attachment?.kind === 'image' && attachment.url ? (
          ...
          >
            <LegacyFaIcon icon="fa-file-lines" className="text-[11px] text-slate-400" />
...
const AgentStudioTrainingPane: React.FC<Props> = ({
  ...
  const handleBatchSendReady = useCallback(async (task: BatchSendTask) => {
    ...
    const historySnapshot: StudioTrainingHistory = messages.map((m) => ({
      role: m.role,
      content: m.content,
      attachments: Array.isArray(m.attachments)
        ? m.attachments.map((a) => ({ name: a.name, kind: a.kind === 'image' ? 'image' as const : 'file' as const, url: a.url, mimeType: a.mimeType }))
        : [],
    }));
    ...
    try {
      for (let batchIndex = 0; batchIndex < task.batches.length; batchIndex++) {
        ...
        const attachmentPayload = task.batches[batchIndex].map((item) => ({
          name: item.name, kind: item.kind === 'image' ? 'image' as const : 'file' as const, url: item.url, mimeType: item.mimeType,
        }));
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const history: StudioTrainingHistory = [...messages, userMsg].map((message) => ({
        role: message.role,
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title="清空当前训练对话与暂存输入"
            >
              <LegacyFaIcon icon="fa-trash-can" className="mr-1.5 text-[10px]" />
              清空对话
            </button>
          </div>
        </div>
            ...
            {sending ? (
              <div className="flex justify-start gap-2.5">
                ...
                <div className="rounded-[18px] border border-slate-200/80 bg-white px-3.5 py-2.5 text-[13px] text-slate-400">
                  <LegacyFaIcon icon="fa-spinner" className="animate-spin text-[11px]" /> 思考中...
                </div>
              </div>
            ) : null}
          </div>
