...
import { createVirtualModel, createVirtualModelVersion, deleteVirtualModel, fetchAdminVirtualModels, publishVirtualModel, replaceVirtualModelVersionAssets, unpublishVirtualModel, updateVirtualModel, uploadInternalAssetStream } from '../../services/internalApi';
...
const VirtualModelLibraryModule: React.FC = () => {
  ...
  const saveEditor = async () => {
    ...
    try {
      ...
      if (editorMode === 'create') {
        ...
        const identityProfile = { description: editorProfile.trim() };
        await updateVirtualModel(selected.id, { code, name });
        if (!selected.version || JSON.stringify(identityProfile) !== JSON.stringify(selected.version.identityProfile)) await createVirtualModelVersion(selected.id, { identityProfile });
      }
      setEditorMode(null);
      await load();
  ...
  const publish = async () => {
    if (!selected) return;
    setPending(true); try { if (selected.status === 'published') await unpublishVirtualModel(selected.id); else if (selected.version) await publishVirtualModel(selected.id, selected.version.id); else { setNotice('请先创建版本并上传 8 张素材'); return; } await load(); } catch (error) { setNotice(error instanceof Error ? error.message : '操作失败'); } finally { setPending(false); }
  };
  ...
  return <><div className="mx-auto flex h-full max-w-[1500px] gap-5 p-5" style={{ color: 'var(--text-primary)' }}>
    <section className="flex min-w-[430px] flex-1 flex-col overflow-hidden border" style={{ borderRadius: 8, background: 'var(--bg-surface)', borderColor: 'var(--border-subtle)' }}>
      <header className="flex items-center justify-between border-b px-5 py-4" style={{ borderColor: 'var(--border-subtle)' }}><div><h1 className="text-lg font-semibold">虚拟模特库</h1><p className="mt-1 text-xs" style={{ color: 'var(--text-tertiary)' }}>管理模特版本与固定参考素材</p></div><button type="button" disabled={pending || deletePending} onClick={openCreate} className="inline-flex items-center gap-2 rounded-md px-3 py-2 text-sm text-white" style={{ background: 'var(--accent)' }}><Plus size={16} />新建</button></header>
