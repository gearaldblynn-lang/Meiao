...
const VirtualModelLibraryModule: React.FC = () => {
  ...
  const upload = async (slot: string, file: File) => {
    if (!selected) return;
    if (selected.version && selected.version.status !== 'draft') { setNotice('已发布版本不可修改，请先新建版本'); return; }
    setPending(true); setNotice('');
    try {
  ...
  const createNextVersion = async () => {
    ...
    setPending(true); setNotice('');
    try {
      await createVirtualModelVersion(selected.id, { identityProfile: selected.version?.identityProfile || {} });
      await load();
    } catch (error) { setNotice(error instanceof Error ? error.message : '新建版本失败'); } finally { setPending(false); }
  };
  const publish = async () => {
    if (!selected) return;
