...
test('buyer show new workflow prepends new sets instead of replacing existing sets', () => {
  assert.match(
    source,
    /sets:\s*\[\.\.\.newSets,\s*\.\.\.\(prev\.sets \|\| \[\]\)\]/,
    'starting a fresh buyer-show generation should keep older generated sets'
  );
  assert.doesNotMatch(
    source,
    /sets:\s*\[\],\s*\n\s*pureEvaluations:/,
