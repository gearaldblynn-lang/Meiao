...
const BuyerShowModule: React.FC<Props> = ({ apiConfig, persistentState, onStateChange }) => {
  ...
  const handleStartWorkflow = async () => {
    ...
    try {
      ...
      void primeWorkspaceCompletionSound(apiConfig);
  ...
  const runSetGeneration = async (set: BuyerShowSet, productUrls: string[], globalRefUrl: string | null) => {
    ...
    try {
        ...
        if (firstRes.status === 'success' && tasks.every((task) => task.status === 'completed')) {
          void playWorkspaceCompletionSound(apiConfig).catch(() => null);
        }
  ...
  const triggerNewKieTask = async (prompt: string, productUrls: string[], refUrl: string | null, isFirstImage: boolean, signal?: AbortSignal, onJobCreated?: (jobId: string, providerTaskId?: string) => void) => {
  ...
  const handleRegenerateTaskOptimized = useCallback(async (task: BuyerShowTask, setId: string, taskIndex: number) => {
    ...
    try {
        ...
        const taskController = createTrackedAbortController(setId);
        const res = await triggerNewKieTask(task.prompt, productUrls, refUrlToUse, isFirstImage, taskController.signal, (jobId, providerTaskId) => {
          onStateChange(prev => ({
            ...prev,
            sets: prev.sets.map(s => s.id === setId ? {
              ...s,
