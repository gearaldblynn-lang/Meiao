...
const AppContent: React.FC<{
  ...
  const productRestoreCancellationRegistryRef = useRef<ReturnType<typeof createProductRestoreCancellationRegistry> | null>(null);
  if (!productRestoreCancellationRegistryRef.current) {
    productRestoreCancellationRegistryRef.current = createProductRestoreCancellationRegistry({
      cancelJob: cancelInternalJob,
      onAudit: ({ projectId, jobIds, cancelledJobCount }) => {
        void safeCreateInternalLog({
          ...
          status: 'interrupted',
          meta: {
            shellProjectId: String(projectId || '').slice(0, 160),
            cancelledJobCount,
            cancelledJobIds: jobIds.map((jobId: string) => String(jobId || '').slice(0, 160)),
          },
        });
      },
    });
  ...
  const resumeProductRestoreProject = useCallback(async (project: Project) => {
    ...
    const cancellationRegistry = productRestoreCancellationRegistryRef.current;
    if (!shouldResumeProductRestoreProject(project, {
      cancelled: cancellationRegistry?.isCancelled(projectId) === true,
    })) return;
    ...
    try {
      ...
      if (!productRestoreContext) {
        ...
        if (analysis.status === 'error') {
          if (shouldStopResume()) return;
          const terminalAnalysisJob = await fetchInternalJob(analysis.jobId || analysisJobId)
          ...
          const failureGenerationContext = cloneGenerationContext(
            storedContext.prompt,
            {
              ...
              ...(terminalAnalysisJob?.status ? { productRestoreAnalysisJobStatus: terminalAnalysisJob.status } : {}),
          ...
          const failedProject: Project = {
            ...
            generationContext: failureGenerationContext,
            status: 'error',
            results: project.results || [],
            completedCount: countCompletedProjectResults(project.results || []),
      ...
      await runProductRestoreFanout({
        ...
        concurrency: apiConfig.concurrency || 1,
        shouldStop: shouldStopResume,
        runItem: ({ target, batchIndex }: { target: Material; batchIndex: number }) => runShellProductRestoreItem({
          input: resumeInput,
          config: resumeConfig,
  ...
  useEffect(() => {
    ...
    projects.forEach((project) => {
      if (shouldResumeProductRestoreProject(project, {
        cancelled: productRestoreCancellationRegistryRef.current?.isCancelled(project.id) === true,
      })) {
  ...
  const handleGenerate = useCallback(async () => {
          ...
          try {
            const sourceDimensions = material.originalWidth && material.originalHeight
              ...
              targetWidth: Number(generationParams.targetWidth || generationParams.width || 0),
	            ...
	            const translationBaseTaskMetadata = {
	              shellProjectId: projectId,
    ...
    const controller = new AbortController();
	    ...
	    const shouldStopProductRestore = () => (
	      ...
	      && (
	        controller.signal.aborted
	        || productRestoreCancellationRegistryRef.current?.isCancelled(projectId) === true
	        || deletedProductRestoreProjectIdsRef.current.has(projectId)
	        || productRestoreProjectDeletionGuardsRef.current.has(projectId)
	      )
	    );
    ...
    try {
                ...
                const analysisProject: Project = {
                  ...
                  error: undefined,
	                ...
	                if (shouldStopProductRestore()) {
	                  recordProductRestoreJobCreated({ projectId, jobId: context.analysisJobId });
	                  controller.abort();
	                  throw Object.assign(new Error('产品还原项目已中断，停止图片任务。'), {
	                    code: 'interrupted',
	                  });
	                }
      ...
      const failedAnalysisJobId = String(
      ...
      const productRestoreFailureParams = {
        ...
        ...(confirmedAnalysisJob?.status
          ? { productRestoreAnalysisJobStatus: confirmedAnalysisJob.status }
          : {}),
        ...(productRestoreFailure.code
          ? { productRestoreAnalysisErrorCode: productRestoreFailure.code }
          : confirmedAnalysisJob?.errorCode
            ? { productRestoreAnalysisErrorCode: confirmedAnalysisJob.errorCode }
            : {}),
      };
      logShellError('shell_generation_failed', error, {
        projectId,
  ...
  const handleRegenerateResult = useCallback(async (
    ...
    try {
      ...
      let retryTaskId = `${resultId}-retry-${Date.now()}`;
      try {
      const isProductRestoreProject = project.module === AppModuleObj.RETOUCH && project.subFeature === 'product_restore';
      const productRestoreWorkflow = isProductRestoreProject
        ? await import('./adapters/shellProductRestoreWorkflow')
        : null;
      if (
        isProductRestoreProject
        && productRestoreWorkflow
        && resultId === productRestoreWorkflow.PRODUCT_RESTORE_MANUAL_REANALYSIS_RESULT_ID
      ) {
        const storedContext = project.generationContext;
        const analysisJobId = String(
          storedContext?.productRestore?.analysisJobId
          || project.backendJobId
          || '',
        ).trim();
        const canManuallyReanalyze = productRestoreWorkflow.canManuallyReanalyzeProductRestore({
          module: project.module,
          subFeature: project.subFeature,
          projectStatus: project.status,
          resultCount: project.results.length,
          analysisJobId,
          analysisJobStatus: storedContext?.params.productRestoreAnalysisJobStatus,
          analysisErrorCode: storedContext?.params.productRestoreAnalysisErrorCode,
        });
        const restoreTargets = (storedContext?.materials.restoreTarget || []) as Material[];
        const productReferences = (storedContext?.materials.productReference || []) as Material[];
        if (!canManuallyReanalyze || !storedContext || restoreTargets.length === 0 || productReferences.length === 0) {
        ...
        const manualParams = {
          ...storedContext.params,
          productRestoreAnalysisSubmissionKey: manualSubmissionKey,
        ...
        setTasks((prev) => [{
          ...
          type: 'plan',
          status: 'generating',
          title: `重新分析: ${project.name}`,
          progress: 8,
          createdAt: Date.now(),
          total: restoreTargets.length,
          completed: 0,
          subFeature: 'product_restore',
        }, ...prev]);

        let analysisContextPersisted = false;
        let manualAnalysisJobId = '';
        let analysisIdentityPersistence: Promise<boolean> = Promise.resolve(true);
        const onManualJobCreated = (jobId: string, providerTaskId?: string) => {
          if (recordProductRestoreJobCreated({ projectId: project.id, jobId })) {
          ...
          setTasks((prev) => prev.map((task) => task.id === retryTaskId ? {
            ...task,
            backendJobId: jobId,
            progress: Math.max(task.progress || 0, analysisContextPersisted ? 18 : 10),
          } : task));
          ...
          latestManualProject = {
            ...latestManualProject,
            backendJobId: jobId,
            planningTaskId: providerTaskId || latestManualProject.planningTaskId,
            generationContext: cloneGenerationContext(
              storedContext.prompt,
              {
                ...manualParams,
          ...
          setProjects((prev) => prev.map((item) => item.id === project.id ? latestManualProject : item));
          analysisIdentityPersistence = persistProjectToSharedState(latestManualProject);
        };
        const mapManualItem = (item: ShellWorkflowImageResult, index: number): GeneratedResult => ({
          id: item.taskId || item.backendJobId || `${project.id}-${item.targetMaterialId || index}`,
          projectId: project.id,
          imageUrl: item.imageUrl || '',
          mediaType: 'image',
          prompt: item.prompt || storedContext.prompt,
          ...
          taskId: item.taskId,
          backendJobId: item.backendJobId,
          batchIndex: item.batchIndex || index,
          targetMaterialId: item.targetMaterialId,
          analysisJobId: item.analysisJobId,
          clientSubmissionKey: item.clientSubmissionKey,
          creditsConsumed: item.creditsConsumed,
        ...
        const syncManualItem = async (item: ShellWorkflowImageResult, index: number, total: number) => {
          ...
          setTasks((prev) => prev.map((task) => task.id === retryTaskId ? {
            ...task,
            type: 'image',
            completed: nextResults.filter((current) => current.status === 'completed').length,
            total,
        ...
        try {
          ...
          const manualResult = await runShellRetouchWorkflow({
            module: AppModuleObj.RETOUCH,
            subFeature: 'product_restore',
            prompt: storedContext.prompt,
            params: { ...manualParams },
            ...
            taskMetadata: {
              shellProjectId: project.id,
              shellProjectName: project.name,
              batchCount: restoreTargets.length,
              subFeature: 'product_restore',
          ...
          latestManualProject = {
            ...
            planningTaskId: workflowError.providerTaskId || latestManualProject.planningTaskId,
            status: 'error',
            results: [],
            completedCount: 0,
      ...
      if (isProductRestoreProject && productRestoreWorkflow) {
        const result = project.results.find((item) => item.id === resultId);
        const storedContext = project.generationContext;
        if (!result || !storedContext) {
          throw Object.assign(new Error(productRestoreWorkflow.PRODUCT_RESTORE_RETRY_CONTEXT_ERROR_MESSAGE), {
        ...
        taskControllersRef.current[retryTaskId] = retryController;
        setTasks((prev) => [{
          id: retryTaskId,
          projectId: project.id,
          module: AppModuleObj.RETOUCH,
          type: 'image',
          status: 'generating',
          title: `重试: ${result.fileName || project.name}`,
          progress: 8,
          createdAt: Date.now(),
          total: 1,
          completed: 0,
        ...
        const retryInput = {
          ...
          taskMetadata: {
            shellProjectId: project.id,
            shellProjectName: project.name,
            batchCount: project.taskCount,
            subFeature: 'product_restore',
          },
          onJobCreated: (jobId: string) => {
            if (recordProductRestoreJobCreated({ projectId: project.id, jobId })) retryController.abort();
            setTasks((prev) => prev.map((task) => task.id === retryTaskId ? {
              ...task,
              backendJobId: jobId,
              progress: Math.max(task.progress || 0, 18),
          ...
          productRestoreContext: storedContext.productRestore,
          publicBaseUrl,
        };
        const retryConfig = buildShellModuleConfig(retryInput);
        const syncRetryItem = async (item: ShellWorkflowImageResult) => {
          const nextResult: GeneratedResult = {
            ...result,
            id: result.id,
            imageUrl: item.imageUrl || '',
            ...
            targetMaterialId: item.targetMaterialId || result.targetMaterialId,
            analysisJobId: item.analysisJobId || result.analysisJobId,
            clientSubmissionKey: item.clientSubmissionKey,
        ...
        const retryResult = await productRestoreWorkflow.runShellProductRestoreSingleRetry({
          ...
          result: {
            id: result.id,
            targetMaterialId: result.targetMaterialId,
            sourceUrl: result.sourceUrl || result.sourcePreviewUrl,
          },
        }, {
        ...
        addToast(retryResult.status === 'completed' ? '产品还原单张重试已完成' : retryResult.status === 'generating' ? '产品还原单张重试已提交，结果待同步' : '产品还原单张重试失败', retryResult.status === 'completed' ? 'success' : retryResult.status === 'generating' ? 'info' : 'warning');
        return;
      }
      if (project.module === AppModuleObj.TRANSLATION) {
        const result = project.results.find((item) => item.id === resultId);
        if (!result) return;
        const completedRetrySubFeature = result.subFeature || project.subFeature || 'main';
        const isSupportedCompletedTranslationRetry = result.status === 'completed'
          && ['main', 'detail'].includes(completedRetrySubFeature)
          && Boolean(result.imageUrl);
        const isSupportedFailedTranslationRetry = result.status === 'error';
        if (!isSupportedCompletedTranslationRetry && !isSupportedFailedTranslationRetry) {
          addToast('该翻译结果不支持单图重试，仅主图出海或详情出海的已完成图片和失败项可重试。', 'info');
          return;
        }
        ...
        try {
        const commitTranslationRetryProjectMutation = (nextResult: GeneratedResult, allowAppend = false) => {
          const commitOutcome = reduceTranslationRetryProjectMutation(projectsRef.current, {
            projectId: project.id,
            retryResult: nextResult,
            allowAppend,
          });
          if (!commitOutcome.updated || !commitOutcome.project) return null;
          projectsRef.current = commitOutcome.projects as Project[];
          setProjects(commitOutcome.projects as Project[]);
          return commitOutcome.project as Project;
        };
        if (
          result.status === 'completed'
          && ['main', 'detail'].includes(completedRetrySubFeature)
          ...
          if (!sourceUrl || !/^https?:\/\//i.test(sourceUrl)) {
            addToast('原图公网地址缺失，无法重新翻译该图片。', 'warning');
            return;
          }
          const translationRetrySnapshot = resolveTranslationRetrySnapshot({
            result,
            projectParams: project.generationContext?.params,
          });
          const snapshotParams = translationSnapshotToParams(translationRetrySnapshot);
          if (!translationRetrySnapshot || !snapshotParams) {
            addToast('该历史结果缺少完整的翻译参数，无法安全重新翻译。', 'warning');
            return;
          }
          ...
          if (snapshotParams.resolutionMode === 'original' && !sourceDimensions) {
            addToast('原图尺寸读取失败，无法按历史参数重新翻译。', 'warning');
            return;
          }
          const modelConfig = {
            targetLanguage: String(snapshotParams.lang || 'English'),
            customLanguage: String(snapshotParams.customLanguage || ''),
            removeWatermark: true,
            aspectRatio: String(snapshotParams.ratio || snapshotParams.aspectRatio || 'auto'),
            quality: String(snapshotParams.quality || '1k'),
            model: normalizeShellImageModel(snapshotParams.model || 'GPT Image 2'),
            resolutionMode: snapshotParams.resolutionMode === 'original' ? 'original' : 'custom',
            targetWidth: Number(snapshotParams.targetWidth || 0),
            targetHeight: Number(snapshotParams.targetHeight || 0),
          ...
          const translationWorkflowModule = await loadShellWorkflowModule();
          const matchedRatio = String(effectiveConfig.aspectRatio || modelConfig.aspectRatio || 'auto');
          ...
          const effectiveRetryParams = {
            ...
            aspectRatio: matchedRatio,
            __workspacePreferences: JSON.stringify(apiConfig.workspacePreferences || getWorkspacePreferences()),
            __retryResultId: retryDescriptor.id,
          ...
          const updateTranslationRetryResult = (nextResult: GeneratedResult, allowAppend = false) => (
            commitTranslationRetryProjectMutation(nextResult, allowAppend)
          );
          const persistTranslationRetryState = async (committedProject: Project) => {
            const committedResult = committedProject.results.find((item) => item.id === retryDescriptor.id);
            if (!committedResult) return false;
            const retryIndex = committedProject.results.findIndex((item) => item.id === retryDescriptor.id);
            await Promise.all([
              persistTranslationFilesToSharedState(
                subFeature,
                [translationResultToFile(committedProject, committedResult, Math.max(0, retryIndex))],
              ),
              persistProjectToSharedState(committedProject),
            ]);
            return true;
          };
          const currentTranslationRetryResult = () => (
            projectsRef.current
              .find((item) => item.id === project.id)
              ?.results.find((item) => item.id === retryDescriptor.id)
          );
          const pendingResult: GeneratedResult = {
            ...result,
            ...retryDescriptor,
            ...
            mediaType: 'image',
            status: 'generating',
            prompt: translationRetrySnapshot.translationGenerationMode === 'AI优化'
              ? '正在重新提取并分析图片文案...'
              : '正在重新翻译当前图片...',
            ...
            translationPlanningText: undefined,
          ...
          const pendingProject = updateTranslationRetryResult(pendingResult, true);
          ...
          setTasks((prev) => [{
            ...
            type: 'image',
            status: 'generating',
            title: `重新翻译: ${result.fileName || result.relativePath || project.name}`,
            progress: translationRetrySnapshot.translationGenerationMode === 'AI优化' ? 8 : 12,
            createdAt: Date.now(),
            total: 1,
            completed: 0,
            subFeature,
          ...
          try {
            await persistTranslationRetryState(pendingProject);
            const { runShellImageGeneration, runShellTranslationPlanningAnalysis } = translationWorkflowModule;
            let planningProviderTaskId = '';
              ...
              runPlanning: async () => {
                const planningResult = await runShellTranslationPlanningAnalysis({
                  ...
                  taskMetadata: {
                    ...
                    shellProjectId: project.id,
                    shellProjectName: project.name,
                    shellResultId: retryDescriptor.id,
                    retryOfResultId: retryDescriptor.retryOfResultId,
                    retryRootResultId: retryDescriptor.retryRootResultId,
                    retryAttempt: retryDescriptor.retryAttempt,
                    sourceOrder: retryDescriptor.sourceOrder,
                    subFeature,
                    sourceUrl,
                  ...
                  const currentRetryResult = currentTranslationRetryResult();
                  if (!currentRetryResult) return;
                ...
                const currentRetryResult = currentTranslationRetryResult();
                if (!currentRetryResult) throw new Error('重试结果已删除，已停止更新。');
                const plannedResult: GeneratedResult = {
                  ...currentRetryResult,
                  prompt: planningText || currentRetryResult.prompt,
                ...
                await persistTranslationRetryState(plannedProject);
                return {
                  ...planningResult,
                  taskId: planningTaskId,
                };
              ...
              runGeneration: async (prompt: string) => {
                ...
                return runShellImageGeneration({
                  ...
                  onJobCreated: (jobId: string, providerTaskId?: string) => {
                    ...
                    const latestRetryResult = currentTranslationRetryResult();
                    if (!latestRetryResult) return;
                    const generationIdentityResult: GeneratedResult = {
                      ...latestRetryResult,
            ...
            if (pipelineResult.status !== 'success' || !pipelineResult.imageUrl) {
              const recoverableResult = {
                ...pipelineResult,
                taskId: pipelineResult.taskId || generationProviderTaskId,
              };
              if (isRecoverableShellWorkflowResult(recoverableResult)) {
                const currentRetryResult = currentTranslationRetryResult();
                if (!currentRetryResult) {
                  setTasks((prev) => prev.filter((task) => task.id !== retryTaskId));
                  delete taskControllersRef.current[retryTaskId];
                  return;
                }
                const syncPendingResult: GeneratedResult = {
                  ...currentRetryResult,
                  status: 'generating',
                  taskId: recoverableResult.taskId || currentRetryResult.taskId,
                  prompt: pipelineResult.prompt || currentRetryResult.prompt,
                  translationPlanningText: pipelineResult.planningText || currentRetryResult.translationPlanningText,
                  translationPlanningTaskId: pipelineResult.planningTaskId || currentRetryResult.translationPlanningTaskId,
                ...
                await persistTranslationRetryState(syncPendingProject);
                addToast('重新翻译任务已提交云端，结果待同步', 'info');
                window.setTimeout(() => void hydrateShellJobs(), 800);
                setTasks((prev) => prev.filter((task) => task.id !== retryTaskId));
                delete taskControllersRef.current[retryTaskId];
                return;
              ...
              const failedResult: GeneratedResult = {
                ...currentRetryResult,
                status: 'error',
                taskId: pipelineResult.taskId || generationProviderTaskId || currentRetryResult.taskId,
                prompt: pipelineResult.prompt || currentRetryResult.prompt,
                translationPlanningText: pipelineResult.planningText || currentRetryResult.translationPlanningText,
                translationPlanningTaskId: pipelineResult.planningTaskId || currentRetryResult.translationPlanningTaskId,
                translationPlanningCreditsConsumed: pipelineResult.planningCreditsConsumed,
              ...
              setTasks((prev) => prev.filter((task) => task.id !== retryTaskId));
              delete taskControllersRef.current[retryTaskId];
              if (failedProject) await persistTranslationRetryState(failedProject);
              addToast(failedResult.error || '重新翻译失败', 'error');
              return;
            }
            ...
            const completedResult: GeneratedResult = {
              ...
              matchedAspectRatio: matchedRatio,
              taskId: pipelineResult.taskId || generationProviderTaskId || currentRetryResult.taskId,
              translationPlanningText: pipelineResult.planningText || undefined,
            ...
            setTasks((prev) => prev.filter((task) => task.id !== retryTaskId));
            delete taskControllersRef.current[retryTaskId];
            if (completedProject) await persistTranslationRetryState(completedProject);
        ...
        const setProjectResult = (nextResult: GeneratedResult) => {
          const committedProject = commitTranslationRetryProjectMutation(nextResult);
          const committedResult = committedProject?.results.find((current) => current.id === result.id);
          if (!committedProject || !committedResult) return null;
          latestFailedRetryResult = committedResult;
          return committedProject;
        };
        const persistFailedTranslationRetryState = async (
          nextProject: Project,
          fileStatus: TranslationBatchFile['status'],
        ) => {
          const committedResult = nextProject.results.find((current) => current.id === result.id);
          if (!committedResult) return false;
          await Promise.all([
            persistTranslationFilesToSharedState(subFeature, [toFileItem(committedResult, fileStatus)]),
            persistProjectToSharedState(nextProject),
          ]);
          return true;
        };

        const failedTranslationWorkflowModule = await loadShellWorkflowModule();
        const pendingResult: GeneratedResult = {
          ...result,
          status: 'generating',
          prompt: retryPrompt,
        ...
        const controller = new AbortController();
        taskControllersRef.current[retryTaskId] = controller;
        const cleanupFailedTranslationRetry = () => {
          setTasks((prev) => prev.filter((task) => task.id !== retryTaskId));
          delete taskControllersRef.current[retryTaskId];
        };
        try {
          ...
          if (failedProject) {
            await persistFailedTranslationRetryState(failedProject, 'error');
          }
          cleanupFailedTranslationRetry();
      ...
      if (subFeature === 'voiceover_translation') {
  ...
  const handleCancelTask = useCallback((taskIdOrProjectId: string) => {
    ...
    const matchingProductRestoreTaskProjectIds = new Set(tasks
      .filter((task) => shellCancelTargetMatches(targetId, collectShellTaskIds(task)))
      .map((task) => String(task.projectId || '').trim())
      .filter(Boolean));
    const matchingProductRestoreProjects = projects.filter((project) => (
      project.module === AppModuleObj.RETOUCH
      && project.subFeature === 'product_restore'
      && (
        matchingProductRestoreTaskProjectIds.has(project.id)
        || shellCancelTargetMatches(targetId, collectShellProjectIds(project))
        || (project.results || []).some((result) => (
          shellCancelTargetMatches(targetId, collectShellResultIds(result))
        ))
      )
    ));
    // Register the tombstone before aborting controllers or collecting job IDs.
    // A racing onJobCreated callback therefore enters the same cancellation aggregate.
    matchingProductRestoreProjects.forEach((project) => {
      productRestoreCancellationRegistryRef.current?.beginCancellation(project.id);
    });
    ...
    const productRestoreCancelJobIds = new Set<string>();
    matchingProductRestoreProjects.forEach((project) => {
      const observedJobIds = Array.from(productRestoreObservedJobIdsRef.current.get(project.id) || []);
      const aggregatedJobIds = productRestoreCancellationRegistryRef.current?.addCancellationJobs(project.id, [
        ...collectShellCancelJobIds(project.id, projects, tasks),
        ...observedJobIds,
      ]) || [];
      aggregatedJobIds.forEach((jobId: string) => productRestoreCancelJobIds.add(jobId));
    });
    const cancelJobIds = Array.from(new Set([
      ...collectShellCancelJobIds(targetId, projects, tasks),
      ...storyboardJobIds,
      ...productRestoreCancelJobIds,
    ]));
    cancelJobIds.forEach((jobId) => {
      if (productRestoreCancelJobIds.has(jobId)) return;
      void cancelInternalJob(jobId).catch(() => null);
    });

    const cancelledProductRestoreProjectIds = new Set(matchingProductRestoreProjects.map((project) => project.id));
    setTasks((prev) => prev.filter((task) => (
      !cancelledProductRestoreProjectIds.has(task.projectId)
      && !shellCancelTargetMatches(targetId, collectShellTaskIds(task))
    )));
    ...
    <ThemeContext.Provider value={{ theme, toggleTheme }}>
      <div className="flex h-screen w-screen overflow-hidden" style={{ background: 'var(--bg-base)', transition: 'background var(--transition)' }}>
        ...
        <div className="flex flex-1 flex-col min-w-0">
          ...
          {pageMode === 'module' && activeModule !== AppModuleObj.AGENT_CENTER && activeModule !== AppModuleObj.AI_CUSTOMER_SERVICE && activeModule !== AppModuleObj.SMART_FACTORY && activeModule !== AppModuleObj.VIRTUAL_MODEL_LIBRARY && activeModule !== AppModuleObj.IMAGE_CROP && !usesDedicatedVideoComposer && (
