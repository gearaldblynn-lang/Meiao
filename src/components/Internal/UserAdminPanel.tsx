...
const UserAdminPanel: React.FC<Props> = ({ enabled, currentUserId = '', onCurrentUserChange }) => {
  ...
  const confirmDeleteSelectedUser = async () => {
    ...
    try {
      ...
      await loadUsers();
      setExpandedUserId('');
      setSuccessMessage(`账号 ${user.username} 已删除，用量统计已保留。`);
    } catch (requestError: any) {
      if (isMissingAccountError(requestError)) {
        setUsers((items) => items.filter((item) => item.id !== user.id));
        setExpandedUserId('');
        setSuccessMessage(`账号 ${user.username} 已不在后端列表，已从当前页面移除。`);
        void loadUsers();
        return;
      }
  ...
  return (
    <section className="mt-8">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">共 {users.length} 个账号，当前显示 {filteredUsers.length} 个。</p>
            </div>
            <div className="flex min-w-[260px] flex-1 items-center justify-end gap-2">
              <input
                ...
                className="h-10 min-w-[220px] flex-1 rounded-2xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-slate-900/10"
              />
              <button onClick={() => void loadUsers()} className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-500 hover:text-slate-900">
                刷新
              </button>
            </div>
          </div>
          ...
          <div className="max-h-[min(560px,calc(100vh-260px))] overflow-y-auto divide-y divide-slate-100">
            {loading ? (
              ...
              <div className="px-6 py-5 text-sm font-bold text-slate-400">{userSearch.trim() ? '没有匹配账号' : '还没有创建其他账号'}</div>
            ) : (
              pagedUsers.map((user) => {
          ...
          {filteredUsers.length > USERS_PAGE_SIZE ? (
            <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50 px-6 py-4">
              <p className="text-xs font-bold text-slate-500">第 {usersPage} / {usersPageCount} 页</p>
      ...
      <ConfirmDialog
        ...
        title="确认删除账号"
        message={confirmDeleteUser ? `确定删除账号 ${confirmDeleteUser.username} 吗？该账号的工作台状态、任务、日志、上传资产和智能体数据会被清理，用量统计会保留。` : ''}
        confirmLabel="确认删除账号"
