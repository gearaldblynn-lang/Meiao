...
test('viral storyboard prompts preserve the required segmented prompt and voiceover format', () => {
  ...
  assert.match(videoStoryboardService, /\[参考视频附件已附加\]/);
  assert.doesNotMatch(videoStoryboardService, /爆款复刻视频公网URL/);
  ...
  assert.match(videoStoryboardService, /人物细节：用一句完整中文描述参考视频中客观可见的人物类型、手部\/身体动作、服装气质和出镜范围，所有分段保持一致；不得识别或复制人物身份/);
  assert.match(videoStoryboardService, /环境\/场景：用一句完整中文描述参考视频中客观可见的具体场景、桌面\/厨房\/办公\/道具、光线方向、景深和机位，并在全片保持连续/);
  assert.match(videoStoryboardService, /最终输出必须替换成从参考视频中观察到的客观描述，不得原样保留“用一句完整中文描述”、不得出现 xxx/);
  ...
  assert.match(videoStoryboardService, /不得逐字转录或复现原视频口播/);
  assert.match(videoStoryboardService, /音效只描述通用类型、情绪和节奏/);
  assert.match(videoStoryboardService, /不得复现歌词、受保护旋律或可识别声音特征/);
  assert.match(videoStoryboardService, /参考视频该分镜口播信息未清晰识别/);
  assert.match(videoStoryboardService, /参考视频该分镜声音类型未清晰识别/);
  ...
  assert.match(videoStoryboardService, /口播（情绪描写）：“xxx（根据参考视频的信息功能和情绪节奏原创改写，不得逐字转录；信息不清写参考视频该分镜口播信息未清晰识别）”/);
  assert.match(videoStoryboardService, /音效：xxx（只描述通用声音类型和节奏，不复现歌词、旋律或可识别声音特征；声音不清写参考视频该分镜声音类型未清晰识别）/);
...
test('video workspace keeps the shell UI while migrating storyboard and diagnosis logic', () => {
  ...
  assert.match(videoStoryboardService, /短视频参考视频结构分析与原创改编导演/);
  assert.doesNotMatch(videoStoryboardService, /短视频爆款拆解复刻导演/);
