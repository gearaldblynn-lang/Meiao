...
import { getWorkspacePreferences } from '../utils/appState';
...
const FileProcessor: React.FC<Props> = ({
  ...
  const workspacePreferences = getWorkspacePreferences(apiConfig);
                ...
                {files.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/30 transition-colors group">
                    ...
                    <td className="px-8 py-4">
                      ...
                      {workspacePreferences.showGenerationProgress && item.status === 'processing' && item.progress < 100 && (
