...
import { normalizeViralProductInfo } from '../utils/videoStoryboardPromptPolicy.mjs';
...
const VIDEO_ANALYSIS_FALLBACK_PRIORITY = ['gemini-3-5-flash', 'gemini-3-flash-openai'];
...
const selectVideoAnalysisFallbackModels = (model: string, chatModels: any[] = []) => {
  ...
  const candidates = (Array.isArray(chatModels) ? chatModels : [])
    ...
    .filter((item) => getModelFamily(item.id) === 'gemini')
    .sort((a, b) => {
      const aIndex = VIDEO_ANALYSIS_FALLBACK_PRIORITY.indexOf(normalizeModelId(a.id));
      const bIndex = VIDEO_ANALYSIS_FALLBACK_PRIORITY.indexOf(normalizeModelId(b.id));
...
const resolveVideoAnalysisFallbackModels = async (videoAnalysisModel: string) => {
  const result = await fetchSystemConfig();
  return selectVideoAnalysisFallbackModels(videoAnalysisModel, result.config.videoAnalysisModels || result.config.agentModels.chat || []);
};
...
const buildScriptRequestPrompt = (
  ...
  if (config.videoGenerationMode === 'viral_split') {
    const productFacts = normalizeViralProductInfo(config.productInfo);
    return `
R Role 角色
你是短视频参考视频结构分析与原创改编导演。

T Task 任务
客观分析所附参考视频的结构，并为当前商品生成原创的“15秒左右为一个分段的连续性宫格分镜提示词”+“分镜对应动态视频脚本提示词”。

C Constraint 约束

输入A：已附加 ${productImageUrls.length} 张商品参考图。商品外观以商品参考图为唯一真实来源，必须保证商品一致性。
输入B：已核验的商品真实信息、卖点、数据等：${productFacts}
输入C：${referenceVideoUrl ? '已附加参考视频文件，请完整读取视频画面和声音。' : '未附加参考视频文件。'}
输出画幅：每个分镜单元格都必须是 ${config.aspectRatio} 视频比例。

核心任务：
- 客观分析参考视频的镜头顺序、节奏、可见人物动作、商品展示、场景、运镜，以及声音类型和节奏；只提取通用结构，不复制受保护表达。
- 按原视频内容自然分段，每段约 15 秒左右。15秒左右为一个分段，不要把关键口播或关键动作硬切在分段尾部。
- 每个分段都必须产出一条“宫格分镜图 prompt”和一条“动态视频脚本提示词”，两者一一对应。
- 根据输入B和参考视频的信息要点重新撰写原创口播；如两者冲突，必须以输入B为准，不得虚构不符合本产品的信息。
- 保持全局一致性，尤其是商品参考图一致性、人物、场景、道具、光影、镜头语言。
- 所有输出里的“商品：”字段必须固定写为“商品：保持与商品参考图完全一致，不展开描述包装细节”。禁止描述、复述或猜测商品包装的颜色、品牌、标签、文字、形状、材质和内容物细节。
- 不得逐字转录或复现原视频口播；只能概括其信息功能和情绪节奏，再根据输入B原创改写。不得新增未经输入B核验的功效、数据或营销承诺。
- 音效只描述通用类型、情绪和节奏，例如背景音乐、环境声、操作声、转场声；不得复现歌词、受保护旋律或可识别声音特征。
- 不得复制原视频中的人物身份、品牌文案、字幕、歌词或可识别声音特征；不得推断敏感个人信息。
- 如果某个分镜的口播信息或声音类型无法清晰识别，必须分别写为“参考视频该分镜口播信息未清晰识别”“参考视频该分镜声音类型未清晰识别”，不得猜测或补写事实。

宫格分镜图 prompt 必须严格使用以下格式，不能压缩成一段话：
分段一
{任务：根据输入按照要求制作一张x（当前分段数量）宫格分镜图，保证每个分镜单元格画面都必须是${config.aspectRatio}的画面比例。
【全片核心视觉基调】
人物细节：用一句完整中文描述参考视频中客观可见的人物类型、手部/身体动作、服装气质和出镜范围，所有分段保持一致；不得识别或复制人物身份
环境/场景：用一句完整中文描述参考视频中客观可见的具体场景、桌面/厨房/办公/道具、光线方向、景深和机位，并在全片保持连续
...
注意：“人物细节”和“环境/场景”后的文字是填写要求，最终输出必须替换成从参考视频中观察到的客观描述，不得原样保留“用一句完整中文描述”、不得出现 xxx。
...
口播（情绪描写）：“xxx（根据参考视频的信息功能和情绪节奏原创改写，不得逐字转录；信息不清写参考视频该分镜口播信息未清晰识别）”
音效：xxx（只描述通用声音类型和节奏，不复现歌词、旋律或可识别声音特征；声音不清写参考视频该分镜声音类型未清晰识别）}
每个分段时间码都从 00:00 开始。

F Format 格式
只输出 JSON 数组，不要 markdown，不要解释。每个数组对象代表一个分段，并严格保留 title、durationSeconds、panelCount、storyboardPrompt、dynamicScriptPrompt 五个字段。
...
10. 当前是原创生成模式，没有参考视频；口播必须优先按“脚本逻辑”中的用户文案逐镜头分配，不得输出任何“参考视频……未识别”文案。用户未提供某镜头口播时应留空，不得伪造视频识别结果。
11. 所有分镜必须符合电商广告逻辑，画面清楚、卖点明确、时间线连续。
12. ${durationRule}
13. 分镜板中禁止字幕、广告语、水印、logo、分镜序号；口播只写在 dynamicScriptPrompt 中。
14. 不得描述、复述或猜测商品包装的品牌、标签、文字和内容物细节；涉及商品时使用“[商品：保持与商品参考图完全一致，不展开描述包装细节]”。
...
export const generateStoryboardScript = async (
  ...
  jobContext: StoryboardJobContext = {},
): Promise<{ script: string; shots: VideoStoryboardShot[]; boards: VideoStoryboardBoard[]; taskId?: string; backendJobId?: string; creditsConsumed?: number }> => {
  ...
  if (safeReferenceVideoUrl) {
    userContent.push({ type: 'text', text: '[参考视频附件已附加]' });
    userContent.push({
      type: 'input_file',
  ...
  onJobCreated?.(job.id, String(job.providerTaskId || '').trim() || undefined);
  const onJobUpdate = (updatedJob: { providerTaskId?: string }) => {
  ...
  let result;
  try {
    result = parseStoryboardPlanningResult({
      content,
      config,
      identitySeed: shellProjectId || job.id,
    });
  } catch (error) {
    logStoryboardEvent('storyboard_script', error instanceof Error ? error.message : '分镜脚本解析失败', 'failed', content.slice(0, 200));
    throw error;
  }

  logStoryboardEvent('storyboard_script', config.videoGenerationMode === 'viral_split' ? '爆款复刻拆解成功' : '分镜脚本生成成功', 'success', '', {
