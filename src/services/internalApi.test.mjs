...
test('retryInternalJob only sends the server-owned provider-attempt confirmation field', async () => {
  const originalFetch = globalThis.fetch;
  const api = await loadInternalApi();
  const requests = [];
  globalThis.fetch = async (url, init) => {
  ...
  try {
    await api.retryInternalJob('job-1');
    await api.retryInternalJob('job-2', {
      confirmNewProviderAttempt: true,
    });
    assert.equal(requests[0].url, '/api/jobs/job-1/retry');
    assert.equal(requests[0].init.body, undefined);
    assert.equal(requests[1].url, '/api/jobs/job-2/retry');
