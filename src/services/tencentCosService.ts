...
const buildUniqueFileName = (file: File, customFileName?: string) => {
  ...
  const sourceName = ensureUploadFileName(customFileName || file.name || 'upload.bin', file.type || '');
...
export const uploadToCos = async (
  ...
  try {
    preparedFile = await prepareImageForUpload(
      file,
      undefined,
      _apiConfig.workspacePreferences?.compressImagesBeforeUpload !== false,
    );
