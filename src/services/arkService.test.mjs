...
test('translation planning prompt supports global translation while preserving logos trademarks and models', () => {
  ...
  assert.match(arkServiceSource, /resolveTranslationTargetMarket/);
  assert.match(translationPlanningSource, /translationScope/);
  assert.match(translationPlanningSource, /subFeature/);
  assert.doesNotMatch(translationPlanningSource, /subFeature: _subFeature/);
  ...
  assert.match(translationPlanningSource, /全部识别并检查，仅在必要时重写/);
  assert.match(translationPlanningSource, /- “xxx”本地化为“xxx”/);
  assert.match(translationPlanningSource, /- “产品主体\/包装实物表面的原文案、压印文字或原图 logo”保持不变/);
  assert.match(translationPlanningSource, /右侧引号内的成品文案只能使用目标语言/);
  assert.match(translationPlanningSource, /目标市场：\$\{targetMarket\}/);
  assert.match(translationPlanningSource, /原文语言与目标语言相同时仍必须执行本地化质量检查/);
  assert.match(translationPlanningSource, /原文已经自然、准确且适合目标市场时/);
  assert.match(translationPlanningSource, /只进行必要修改/);
  assert.match(translationPlanningSource, /不得为了体现优化而强制改写/);
  assert.match(translationPlanningSource, /结构说明语言不得作为成品文案语言/);
  assert.match(translationPlanningSource, /E Example 示例/);
  assert.doesNotMatch(translationPlanningSource, /用中文逐条输出/);
  assert.match(translationPlanningSource, /runTranslationPlanningWithLanguageGuard/);
  assert.match(translationPlanningSource, /目标市场的母语电商本地化文案总监/);
  ...
  assert.match(translationPlanningSource, /说明书语气过重/);
  ...
  assert.match(translationPlanningSource, /根据当前图片类型和文案实际角色选择对应规则/);
  assert.match(translationPlanningSource, /副标题自然补充卖点，不得机械重复主标题/);
  assert.match(translationPlanningSource, /图标标签保持短小/);
  assert.match(translationPlanningSource, /参数、数字和单位默认原样保留，不得换算数值或计量单位/);
  assert.match(translationPlanningSource, /不得改变数字字符、正负号、小数位、日期含义或单位字符/);
  assert.match(translationPlanningSource, /只允许调整周边空格、标点和不改变字符的视觉排版/);
  assert.match(translationPlanningSource, /自然、正确和信息完整优先于字符数/);
  assert.match(translationPlanningSource, /不通过删除必要助词、谓语或指代对象强行压缩/);
  assert.doesNotMatch(translationPlanningSource, /80%-120%/);
  assert.doesNotMatch(translationPlanningSource, /仍必须本地化改写/);
  assert.doesNotMatch(translationPlanningSource, /均重写/);
  assert.doesNotMatch(translationPlanningSource, /再剥离原句语序/);
  assert.doesNotMatch(translationPlanningSource, /画面主题和版式位置保持不变/);
  assert.match(translationPlanningSource, /不得新增原图没有的功效、认证、材质、产地、健康功效、使用效果、价格或促销信息/);
  assert.equal((translationPlanningSource.match(/不得猜测不可读文字/g) || []).length, 1);
  assert.ok(
    translationPlanningSource.indexOf('不得猜测不可读文字') > translationPlanningSource.indexOf('${scopeConstraintBlock}'),
    'unreadable-copy protection must apply after either translation scope is selected',
  );
  assert.equal((translationPlanningSource.match(/不得新增原图/g) || []).length, 1);
  assert.equal((translationPlanningSource.match(/commentary、final_answer/g) || []).length, 1);
  assert.doesNotMatch(translationPlanningSource, /runTranslationNativeCopyReview/);
  assert.doesNotMatch(translationPlanningSource, /REVIEW_STATUS:/);
  assert.doesNotMatch(translationPlanningSource, /translation_native_copy_review/);
  assert.match(translationPlanningSource, /description: planning\.content\.trim\(\)/);
  assert.match(translationPlanningSource, /creditsConsumed: planning\.creditsConsumed/);
  assert.match(translationPlanningSource, /taskId: planning\.taskId/);
});
