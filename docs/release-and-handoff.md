...
## 16. 2026-07-28 虚拟模特八视角生成与 004/005 安全集成

- 集成边界：不执行迁移包的整树源码覆盖，以当前发布分支为唯一代码基线，只窄接入八视角生成领域层、管理员 API 和新建向导。管理页继续只有左侧“新建”，已发布模特不提供“新建版本”，身份资料与八张固定素材保持不可修改。
- 生成合同：新建资料后可手动上传八张或自动生成。自动生成先提交 C01/P01，两个基准输出完成解码、永久化并得到稳定引用后，才允许提交其余六张；失败不会自动创建新的付费任务，只有用户显式 retry/regenerate 才会产生新 attempt。当前验收只使用依赖注入桩，没有创建真实计费 provider 任务。
- 数据合同：安全导入器默认纯 dry-run，校验三份 manifest 与 80 个文件，只添加稳定 ID 缺失的 004/005、2 个版本、16 个槽位、32 条 registry 和 32 个文件。001–003 同 ID 数据只审计、不覆盖；活动 code 异 ID fail closed；导出 loopback URL 按目标环境重建。
- 本地数据证据：停服且 active jobs 为 0 后执行显式 offline import，数据总量从 models/versions/relations/registry `5/5/25/232` 增加到 `7/7/41/264`，published 模特从 3 增至 5。001–003 的模型、版本、24 个槽位、48 条 registry 和 48 个文件 hash 全部不变；第二次 dry-run 五类新增均为 0；后端 health、Temporal worker 均恢复正常。
- 发布状态：`not_deployed`。GitHub 与腾讯云仍需在全量 `npm run verify`、最终代码审查和发布数据 dry-run 后分别执行。云上写入后必须验证 001–005、普通同事读取权限、Node/正式域名素材 200 与字节/哈希一致；不得用 health 或本地数据成功代替真实公网素材/UI 验收。
