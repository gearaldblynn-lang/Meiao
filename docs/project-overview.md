...
- `VITE_MEIAO_PRODUCT_REPLACE_GENERATION_PROMPT_MAX_CHARS`：默认 `18000`；限制产品替换执行提示词的最大字符数。策划 v6 只投影素材编号、数值区域和局部融合决策，产品身份以上传图像为真值；超限在付费生图提交前停止，不得丢弃图像输入绕过。该变量是前端构建期配置，修改后必须重新构建。
- `VITE_LOGO_REPLACE_GENERATION_PROMPT_MAX_CHARS`：默认 `18000`；限制 Logo 替换执行提示词的最大字符数。策划 v4 只投影数值区域和表面融合决策，Logo 素材图是原子图稿真值；超限时在付费生图提交前拒绝。该变量必须低于当前 provider 真实文本上限，是前端构建期配置，修改后必须重新构建。
