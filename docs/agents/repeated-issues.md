...
- Cloud validation: 生产发布必须等真实任务归零并走标准 drain/mutex/release health，禁止 active-job override。发布后用上述两张董丹丹 styleRef 完成 Bearer API 自动补 Cookie、原生 Cookie GET、Bearer fetch、Node/Nginx、MIME、字节与 SHA-256 一致性验证；资源探针通过后仍要把“真实 DOM 缩略图和全屏预览均 `complete=true`、`naturalWidth>0`”单列为 UI 观察证据，不能混写。
