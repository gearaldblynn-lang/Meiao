...
- Fix: 所有 buffer 和 streamed-file 结果落盘统一生成 HMAC `asset_key` URL，使未来图片、音频和视频可直接安全读取；登录及 `/api/auth/me` 同步下发仅作用于 `/api/assets/file/` 的 HttpOnly、SameSite=Strict 素材会话 Cookie，兼容历史无签名结果和 apex/www，不允许其他 API 使用 Cookie 鉴权。退出登录按同一路径清除。
- Regression check: `node --test server/managedAssetSessionCookie.test.mjs server/assetStore.test.mjs server/managedAssetReadRoute.test.mjs`；`npm run verify`。生产验收必须用原故障 asset 分别证明 Bearer→`/api/auth/me` 设置 Cookie、仅 Cookie 的公网查看/下载返回同一媒体字节与哈希，并用带签名 URL 在无会话下验证未来路径。
