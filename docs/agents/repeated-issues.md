...
- Fix: 托管素材身份解析器严格提取 safe asset ID，任何保留 `managed:` scheme 的非规范形式直接拒绝；统一读取解析器继续执行 active、未删除、非空真实 owner 校验，并要求资产行 `publicUrl` 是同一 asset ID 的 `/api/assets/file/...` 路径，之后才签发本机 capability。provider 暂存以恢复后的 URL 推导文件名和 MIME，不对外暴露 capability 或无凭证正式域名直链；resolver 缺失、返回空或资产行路径失配时必须在任何 fetch/upload 前 fail closed。
