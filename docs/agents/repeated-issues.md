...
## 2026-07-29 - 浏览器直连站内结果时必须具备可用的素材读取凭证
...
- Root cause: 公共模特素材安全加固把 owner/access-key 校验前移到所有 `/api/assets/file` 存储分支，却只给虚拟模特 UI 增加了认证 fetch。普通项目卡仍由原生 `<img>` 和浏览器下载直接请求历史无签名 URL，无法携带 localStorage 中的 Bearer token，因此内部结果统一被拒绝。
- Fix: 所有 buffer 和 streamed-file 结果落盘统一生成 HMAC `asset_key` URL，使未来图片、音频和视频可直接安全读取；登录及 `/api/auth/me` 同步下发仅作用于 `/api/assets/file/` 的 HttpOnly、SameSite=Strict 素材会话 Cookie，兼容历史无签名结果和 apex/www，不允许其他 API 使用 Cookie 鉴权。退出登录按同一路径清除。跨 apex/www 的单图或 ZIP 下载代理只对同一站点的 `/api/assets/file/` 转发当前 Bearer，外部 provider、其他本站路由和伪造相似域名绝不携带凭证。
- Regression check: `node --test server/assetDownloadProxyAuth.test.mjs server/assetDownloadProxy.test.mjs server/managedAssetSessionCookie.test.mjs server/assetStore.test.mjs server/managedAssetReadRoute.test.mjs`；`npm run verify`。生产验收必须用原故障 asset 分别证明 Bearer→`/api/auth/me` 设置 Cookie、仅 Cookie 的公网查看返回同一媒体字节与哈希、从 www 经同源下载代理读取 apex 历史 URL，以及用带签名 URL 在无会话下验证未来路径。
- Cloud validation: release `meiao-20260729110742-0770733995d6`。原故障 asset 在 apex、www、Node、历史 Cookie、未来签名和跨域下载代理路径均为 200 `image/jpeg`、167787 bytes、同一 SHA-256；未登录仍 403，非素材代理目标仍 401。运行文件哈希一致，应用根目录 0755、`.env.server` 0600，deploy marker/mutex 均不存在。
- Avoid next time: 收紧素材授权必须建立“记录身份、服务端授权、浏览器传凭证、Nginx/对象存储交付”四层合同。任何原生 `<img>/<video>/<a download>` URL 都不能假设会携带 localStorage Bearer；安全修复必须回放历史无签名数据，并覆盖查看、下载、apex/www 和全部落盘入口，不能只验证新上传或某一个 UI 模块。
