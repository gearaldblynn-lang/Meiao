...
- Cloud validation: 第一次 release `meiao-20260729110742-0770733995d6` 只证明原故障 asset 在人工 Cookie/签名/代理请求中为 200，不能视为真实 UI 验收。二次发布必须在“多桑”登录态刷新后，逐个核对项目 2–9 的实际 DOM 图片为 `blob:`、`complete=true`、`naturalWidth>0`，并触发一次项目下载确认成功；同时复核未登录原 URL 仍为 403。
