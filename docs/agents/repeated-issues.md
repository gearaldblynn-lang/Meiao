...
- Regression check: `node --test server/managedAssetAccessKey.test.mjs server/providerAssetTransfer.test.mjs server/managedAssetReadResolver.test.mjs server/managedAssetReadRoute.test.mjs`、virtual-model/provider targeted suites 与完整 `npm run verify` 均通过，二次独立复审为 Critical/Important/Minor 全 0。release `meiao-20260729165941-6e98d573260d` 上以韩系女生 `001 / v2` 的真实 `front_close` 资产验证：loopback capability 绑定实际 owner、GET 200、MIME/678986 字节与数据库一致；非付费模拟暂存只返回外部 staged URL；匿名正式域名继续 403。源码哈希、health、worker、COS、目录权限与前端资源链通过；未创建付费 canary。
...
- Fix: 托管素材身份解析器严格提取 safe asset ID，任何保留 `managed:` scheme 的非规范形式直接拒绝；统一读取解析器继续执行 active、未删除、非空真实 owner 校验，并要求资产行 `publicUrl` 是同一 asset ID 的规范 `/api/assets/file/<id>[/<filename>]` 路径。点段、编码点段、额外层级、尾随空段和 URL 规范化前后不一致的路径都在 capability 签发前拒绝。provider 暂存以恢复后的 URL 推导文件名和 MIME，不对外暴露 capability 或无凭证正式域名直链；resolver 缺失、返回空或资产行路径失配时必须在任何 fetch/upload 前 fail closed。
- Regression check: `server/managedAssetReadResolver.test.mjs` 锁定 stable identity 到 owner-bound loopback MP4，以及跨账号、缺 user/owner、inactive/deleted、query/hash 伪装输入全部拒绝且不 HEAD/签名；`server/providerAssetTransfer.test.mjs` 锁定 Golden 暂存收到 `video/mp4` 和 `.mp4` 文件名，并锁定 resolver 缺失/空结果、资产行空路径/错路径/错 ID、非规范 `managed:` scheme、点段/编码点段/额外层级/尾随空段时 capability/fetch/upload 均为 0。发布后还必须用故障素材做不创建 provider 任务的解析/FFprobe 探针，再用单次确认的真实 canary 核对 Golden、Gemini、逐段 TTS 和最终 MP4。
...
## 2026-07-31 - Golden 暂存地址不能替代 owner 受控源做服务端媒体探测

- Symptom: `managed://` 路由修复发布后，唯一真实口播 canary 仍在 `input_prepared` 以 `media_process_failed` 失败；父任务无 provider task，Golden 子任务无 provider task 且残留 `running`。
- Environment: Tencent Cloud production / voiceover translation with text removal / Golden pre-submit media preparation.
- Root cause: 去字幕适配器先把内部视频暂存成外部 provider 地址，再对暂存地址运行本机 FFprobe。相同源素材通过 owner-bound 读取地址可稳定解码为 22.221 秒 H.264/AAC，但暂存地址探测失败。`media_process_failed` 又未被 child ledger 视为确定性失败，父任务结束后子任务仍占用运行态。
