...
## 2026-07-28 - 口播分段不能合并慢放，纯新口播成品不得重新混入任何原音轨
...
## 2026-07-29 - 项目结果必须通过统一鉴权图片边界读取

- Symptom: 多桑项目 `proj-plan-1785289103350` 的任务和素材记录均成功，但项目卡破图，查看和下载同一结果都失败；公网与 Node 直连素材 URL 均返回 403。
- Evidence: job `d01cdbdd39c5067301bb3950`、provider task `5a96f509cc9475dfe3da26e60b79e157` 已成功，asset `3123fa51e2c46f437d3b2bc2` 为 active JPEG，167787 bytes，SHA-256 `91eb83bc5e039e6a1c795913b1165e71e3212d08e00eb4eacee82257ceb41132`。文件、数据库、provider 和目录权限均正常，故障发生在 Node 授权边界，不是再次出现 X-Accel 权限问题。
- Root cause: 公共模特素材安全加固把 owner/access-key 校验前移到所有 `/api/assets/file` 存储分支，却只给虚拟模特 UI 增加了认证 fetch。普通项目卡仍由原生 `<img>` 请求历史无签名 URL，无法携带 localStorage 中的 Bearer token，因此内部结果统一被拒绝。第一次修复只增加 Cookie、下载代理和未来结果签名 URL，没有替换项目卡真实消费边界；人工构造 Cookie/签名请求成功被错误当成 UI 已恢复。
- Fix: `ProjectCard` 的网格预览、详情大图、翻译版本缩略图及 `PlanEditor` 的结果图全部走 `AuthenticatedAssetImage`，managed asset 先以当前 Bearer 获取 Blob，再通过 object URL 渲染；查看与单图/ZIP 下载共享 `fetchImageBlobWithProxy`。资产记录恢复只持久化稳定无签名 URL，禁止把 HMAC `asset_key` 写入 buffer、stream 或 COS 的 `public_url`；素材 Cookie 仅作为尚未迁移的原生媒体兼容层。
- Regression check: `node --test src/shell/components/ProjectCard.authenticatedAssetImage.test.mjs server/assetStore.test.mjs src/utils/imageUtils.test.mjs server/assetDownloadProxyAuth.test.mjs server/assetDownloadProxy.test.mjs server/managedAssetSessionCookie.test.mjs server/managedAssetReadRoute.test.mjs`；`npm run verify`。源码合同禁止项目结果退回 `<img src={result.imageUrl}>`，资产存储合同禁止任何配置下持久化 `asset_key`。
- Cloud validation: 第一次 release `meiao-20260729110742-0770733995d6` 只证明原故障 asset 在人工 Cookie/签名/代理请求中为 200，不能视为真实 UI 验收。二次 release `meiao-20260729113350-be3b4a1f4714` 已确认公网加载 `ShellMigratedApp-HbQtLgIR.js`；“多桑”项目 1–9 的精确结果资产逐一通过匿名 403、owner 签名 200、`image/jpeg`、字节和 SHA-256 一致、active 且未删除。当前受控浏览器登录态的首图列表为空，并非故障账号项目视图，因此资源链通过仍不能冒充 UI 完成；最终必须在“多桑”登录态刷新后逐个核对实际 DOM 图片为 `blob:`、`complete=true`、`naturalWidth>0`，并触发一次项目下载确认成功。
- Avoid next time: 收紧素材授权必须建立“记录身份、服务端授权、浏览器传凭证、UI 统一消费、Nginx/对象存储交付”五层合同。原生 `<img>/<video>/<a download>` 不能假设会携带 localStorage Bearer，签名 capability 不得持久化；安全修复必须回放历史无签名数据，并用故障账号的真实 DOM 和下载动作验收，Curl 200、provider 成功或数据库记录正常都不能替代最终用户路径。
