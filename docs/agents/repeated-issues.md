...
- Root cause: `.gitignore` only excludes untracked paths. It cannot protect a literal that is pasted into a tracked Markdown file, so treating the repository ignore rules as a secret boundary was unsafe. A normal review diff of the removal also retains the deleted line, so saving that diff as an ignored scratch artifact creates another plaintext copy that the tracked-file scanner cannot see.
