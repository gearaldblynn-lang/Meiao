...
版本基准：旧实现 `260407A`；最新重写目标见第 0 节。

本文档目标不是介绍产品，而是把“买家秀”的提示词、状态结构、执行顺序和异常处理整理成可复刻规格。第 0 节记录最新重写目标；后续章节保留旧实现细节，用于迁移对照和避坑。

## 0. 最新重写目标速览（2026-07-02）

> 本节按 `开发文档/买家秀-重写开发文档.md` 的最新定稿整理。下文第 1-16 节仍保留旧 `BuyerShowModule.tsx` / Shell 迁移链路的历史复刻说明；读新买家秀时，以本节为准。

### 0.1 关键代码入口

- 仍复用模块入口：`AppModule.BUYER_SHOW`，不新增第二个买家秀入口。
- 可复用底层任务提交：`src/services/internalApi.ts` 的 `createInternalJob`
- 可复用素材上传：`src/adapters/shellWorkflow.ts` 的 `uploadShellMaterial`
- 可复用/仿写策划函数：`src/services/arkService.ts` 的 `generateBuyerShowPrompts`
- 可复用结果展示：`src/shell/modules/BuyerShow/BuyerShowModule.tsx` 背后的 `ProjectListView`
- 目标替换对象：旧 `src/adapters/shellWorkflow.ts` + `src/shell/components/layout/BottomInputBar.tsx` 的买家秀耦合实现，最终应被新实现替换。

### 0.2 新功能定位

买家秀 = 用 AI 生成“仿真买家秀”图。输入拆成三类角色：

- 产品图：画面主体，必须真实保留。
- 模特图：使用产品的人物参考，可选，相似而不是一比一复刻。
- 场景参考：环境 / 背景 / 氛围参考，可选，相似而不是一比一复刻。

它仍然是两阶段流水线：

1. 策划阶段：每套单独生成一组拍摄方案，每张图一段 prompt，第 1 张是本套基准图。
2. 出图阶段：按方案逐张提交 `kie_image` 任务；首张完成后，其结果作为本套后续图片的 benchmark。

### 0.3 多套逻辑

最新逻辑里，“多套”不再主要依赖固定 `setIndex` 场景发散，而是依靠用户上传/填写多套独立输入。
...
- 模特图：最多 1 张，可选。
- 场景参考：最多 4 张，可选。
- 本套文字要求：一段文本，可选，是这套差异化的核心输入。
- 本套出图张数：每套可不同，至少 1 张。

关键约束：

- “套 = 一种产品 / 一组独立要求”，不是同一产品的固定四种场景枚举。
- 素材从上传那一刻就必须钉死归属：`setIndex + type(product/model/scene)`。
- 展示序号可以因删除重排，但内部稳定 `setIndex` / set id 不能重用，否则历史结果会错位。
- 产品图是唯一必选项；无模特图就按目标市场和文字要求生成人物；无场景参考就按市场和文字要求生成场景；仅静物模式禁止人物。

### 0.4 策划提示词结构

策划阶段仍使用 RTCFE 结构，但入参要按“单套”组织：

- `R Role`：目标市场的电商 UGC / Buyer Reviews 策划专家。
- `T Task`：基于“本套产品图 + 本套文字要求 + 可选模特图/场景图”，生成 `imageCount` 个真实买家秀拍摄方案，并输出一条本地语言评价。
- `C Constraint`：真实、干净、iPhone 日常感；禁止脏乱差；产品必须保真；一套图要覆盖场景、细节、使用/互动；按全局 `includeModel` 走真人或静物策略。
- `F Format`：只输出 JSON，结构为 `tasks[].prompt`、`tasks[].style`、`tasks[].hasFace`、`evaluation`。
- `E Example`：给一个整洁桌面自然光示例。

策划 prompt 的输入优先级：

1. 本套产品图：唯一真实主体，不能替换、改包装、改品牌、改结构。
2. 本套文字要求：动作、风格、卖点、目标人群、使用场景等，作为这套差异化的最高优先级文本。
3. 本套场景参考：只决定环境、光线、道具、色调和氛围，不替代产品主体。
4. 本套模特图：只参考脸部气质、年龄段、姿势、手部动作、穿搭方向和拍摄状态；输出人物仍要符合目标市场。
5. 全局目标市场 / 含模特开关：决定语言、人群、人物出现规则和本地化生活感。

如果直接复用 `generateBuyerShowPrompts`，需要把“本套文字要求”并入 `state.productFeatures` / `userRequirement`，把本套产品图作为 `productUrls`，把本套场景参考首图优先作为 `referenceUrl`。`setIndex` 只作为轻量发散兜底，不再承担多套差异的主责任。

### 0.5 生图提示词结构

生图 prompt 按每张任务组装，核心仍是 5 块：

1. 真实 iPhone 买家随手拍母规则：非棚拍、非专业构图、轻微镜头畸变、不完美取景、自然环境光、真实生活感。
2. 真人 / 静物分支：真人必须像目标市场本地真实用户；静物则禁止人物，产品自然放在日常环境里。
3. 参考图说明：首图使用本套场景/模特参考；后续图使用本套首张已生成结果作为 benchmark，并要求换构图、机位、动作焦点和产品叙事。
4. 包装保真硬约束：包装身份、细节、标签、版式、品牌标识、色块、可见元素都不能改写、重设计、简化或编造；产品真实尺寸、透视、接触、阴影、遮挡必须融入场景。
5. 任务场景：`SCENE:` 或 `NEXT SHOT:` 后接策划模型返回的英文 `task.prompt`。
...
- 本套产品图全部放入。
- 加入本套第 1 张已生成成功的 benchmark 图。
- 必要时可继续附本套模特/场景参考，但 benchmark 是同套一致性的第一锚点。

### 0.6 最新套图提交与恢复逻辑

最新目标流程：

1. 用户创建 N 套；每套独立上传产品图、模特图、场景参考，并填写本套文字要求和本套出图张数。
2. 提交前校验：每套至少 1 张产品图；目标市场必选；本套出图张数至少 1。
3. 对每套独立策划：一套失败只影响这一套，不能中断其它套；瞬时错误可重试。
4. 策划结果里第 1 张是 benchmark。含模特模式下，如果模型返回的第 1 张不是人像基准，可把第一个 `hasFace=true` 的任务提到首位。
5. 对每套先提交第 1 张 `kie_image`，等待其完成并取 `resultUrl`。
6. 把第 1 张结果写入 `benchmarkUrl`，再提交同套后续图片。
7. 每套结果组织成一个正常 project 交给 `ProjectListView` 展示；一套图 = 一个项目任务卡。
8. 任务 payload 必须带归属元数据：`setIndex` / set id、`indexInSet`、`isBenchmark`、`imageCount`、`shellProjectId`、`shellProjectName`、`buyerShowDisplayPrompt`、`buyerShowEvaluation`。

状态结构建议只保留 `sets[]` 作为单一真相；不要同时维护顶层 `tasks[]` 和 `sets[].tasks` 两套数据。刷新恢复时以后台 job + set 归属元数据重建项目卡，避免旧快照覆盖新 job 结果。
