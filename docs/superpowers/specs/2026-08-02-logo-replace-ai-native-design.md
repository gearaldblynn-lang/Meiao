...
日期：2026-08-02（历史 v6 设计）

> 2026-08-05 已由 v7 语义定位与分模式终态保护取代：用户框选不再是硬裁切边界，策划识别真实 `targetBounds`，物体表面 Logo 使用 `surface_integrated`，二维角标使用 `graphic_overlay`，局部候选以 `editEnvelope` 羽化合成且包络外恢复原图。当前合同以 `docs/prompt-rtcfe-migration-map.md` 的 P26 和 `CLAUDE.md` 根因 #100 为准；下文仅保留为历史设计背景，不得据此恢复“完整包住选框”或“所有场景精确叠加”的旧规则。
