...
日期：2026-08-02（2026-08-03 升级为 v4：保留 v3 选框预检和几何合同，取消出图后 AI 审查）
...
3. 程序生成带编号的区域标记图。
4. 程序为每个绑定 Logo 生成只裁外围空白的紧边界身份参考，并记录可见图稿长宽比。
...
8. 生图模型直接输出最终整图。
9. 程序完成原尺寸归一化和受管资产转存。
10. provider 生图成功且最终资产处理成功后，结果直接进入完成态；不创建出图后 AI 审查任务，不再由第二个模型覆盖成图状态或额外计费。
11. 用户直接查看最终结果并决定是否满意；需要调整时由用户修改选框、Logo 或替换要求后主动重试。

最终链路禁止再使用浏览器 Canvas 清理结果、贴回透明 PNG 或局部保护合成。
...
紧边界身份参考只移除素材外围无意义的透明或纯色空白画布，避免原始画布比例被误当成 Logo 结构。它保留文字、图形、颜色、内部间距，以及具有可见边界、形状、颜色或纹理的有意底板；该参考同时供分析、生图和质检使用，不参与浏览器后贴或局部合成。
...
3. 最终资产处理只校验整图比例、尺寸和持久化，不评判 Logo 是否“看起来满意”。
...
1. 下载 provider 返回的完整图片 Blob，不截取区域，不生成遮罩，不重新贴 Logo。
...
4. 比例一致但像素尺寸不同，允许只对完整结果图做一次高质量等比缩放，恢复到原图画布宽高。这个 Canvas 仅承担整图尺寸归一化，不参与 Logo 清理、局部合成或覆盖。
5. 将归一化后的完整图片上传为站内受管结果资产，不能把 provider 临时 URL 当长期结果。
6. 最终 URL 改变后回写原 backend job 的 `result.imageUrl`，同时保留 `originalProviderImageUrl` 作为来源证据；回写失败不能把当前已正确显示的最终图回退为 provider 原图。

这一步解决的是交付稳定性和画布合同，不改变 AI 原生替换原理。Logo 身份、清理和融合仍全部由生图模型一次性完成。
...
生成式编辑不能提供 Logo 字符逐像素完全一致的数学保证。当前方案通过选框覆盖预检、紧边界身份参考、v3 结构合同、数值几何合同、原子图稿约束和固定映射提高准确率；最终是否满意由用户直接查看成图决定。若结果不合格，用户可调整要求后主动重试，系统不回退到浏览器硬贴图，也不增加出图后 AI 裁判。
