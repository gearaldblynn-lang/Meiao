...
> **2026-08-01 合同替代说明：** 当前活动合同以 `docs/project-overview.md`、`docs/release-and-handoff.md`、`docs/tencent-cloud-deploy.md` 及 `CLAUDE.md` 的口播翻译根因条目为准。本历史规格中“保留背景音乐/环境音”“短口播保持 `atempo=1`”“混入 `no_vocals`”等决策已经废止，不得用于实现、测试或发布验收。当前成品只保留新口播；短口播允许在安全下限内减速并在原时间窗居中。
...
- 创建入口不重新设计页面：必须保持现有视频生成底部 Composer 的大输入框、素材预览、胶囊参数栏和右侧提交按钮形式。
- 页面主体只展示项目与结果，不再放置目标语言、翻译方式、音色或去文案的大块配置卡片。
- 大输入区仅负责单视频拖拽/点击上传、上传与转码进度、视频预览和替换/清除，不新增自由文本提示词。
- 目标语言、翻译方式、音色和去文案全部位于大输入区下方的胶囊参数栏；去文案区域编辑由对应胶囊打开浮层完成。
...
- 新增 `voiceover_translation` 视频子功能、独立项目结果视图和项目卡；创建入口复用现有视频生成底部 Composer。
...
1. 用户进入「短视频 → 口播翻译」。
2. 在现有视频生成样式的大输入区点击或拖拽上传一个视频。
3. 系统读取时长、分辨率、容器、编码和音轨。
4. 用户在大输入区下方的胶囊参数栏选择目标语言、翻译模式和音色模式。
5. 用户可在同一参数栏开启「同时去文案」，并从该胶囊打开区域编辑浮层调整默认底部 30% 区域。
6. 用户点击「开始口播翻译」。
...
- `MODULE_SUB_FEATURES[AppModuleObj.VIDEO]` 顺序调整为：
  ...
  - `diagnosis`：视频诊断
- `VideoModule` 在 `voiceover_translation` 下继续使用项目列表展示历史任务与结果；`VoiceoverTranslationWorkspace` 只负责口播翻译草稿、媒体准备和提交流程，不再渲染页面级配置卡片。
- 从现有 `BottomInputBar` 提取小型共享 Composer 外壳/视觉原语，标准视频生成和口播翻译共同使用同一套最大宽度、圆角、边框、间距、胶囊选择器和右侧提交按钮。不得在口播翻译组件内复制一套近似样式。
- 口播翻译创建入口挂载到 `voiceover-translation-composer-slot`，宽度保持 `max-w-[896px]`，位置与标准视频生成输入器一致。
- 页面主体只展示项目列表、空状态和结果；不得再出现「目标语言」「口播音色」「同时去文案」等独立大卡片。
- 大输入区没有 prompt textarea，整体高度和边界仍沿用标准 Composer：
  - 无素材时，整个大输入区是单视频点击/拖拽上传目标。
  - 上传或转码时，原位显示真实进度和当前阶段。
  - 准备完成后，原位显示视频缩略图、文件名、权威时长、分辨率、替换和清除入口。
- 功能未就绪、账号无权限或媒体准备失败时，原因显示在 Composer 内，不另建页面级警告面板；历史项目仍可查看和下载。
- 页面遵守项目既有自然滚动规则，不使用会裁掉内容的固定高度内部滚动区。
- 首版只允许一个活动草稿和一个视频；选择第二个视频时要求先替换当前视频。
- 宽屏下参数栏与右侧提交按钮保持一行优先；窄屏允许参数胶囊自然换行，不得横向裁切。浏览器验收至少覆盖默认视口和 `820px`。
...
所有配置使用标准 Composer 底部胶囊选择器，顺序固定为：

1. `目标语言`：当前值显示中文名称；弹层先列常用语言，再列同一版本化目录中的其余语言。
2. `翻译方式`：`自然口播 | 忠实直译`。
3. `口播音色`：`自动匹配` 与当前服务端音色目录中的预设音色合并在一个选择器中。
4. `去文案`：`关闭 | 开启`；开启后可从同一胶囊进入区域编辑浮层。

前端不得自行添加 provider 未支持的语言代码或音色。预设音色可显示简短特点，但不得宣称克隆原声。自动音色的实际选择在分析完成后写入项目卡。

右侧按钮文字固定为「开始口播翻译」。未上传视频、媒体仍在准备、目标语言为空、预设音色无效、功能未就绪、账号无权限或正在提交时必须禁用。付费确认弹窗继续显示视频时长、语言、翻译方式、音色、去文案和可能产生费用的阶段。
...
- 口播翻译子功能路由、共享底部槽位和普通视频 prompt 输入器排除。
- 标准视频生成与口播翻译共同调用共享 Composer 外壳/视觉原语；源码门禁拒绝口播翻译重新出现页面级语言、音色或去文案配置卡片。
- 大输入区的空态拖拽、上传进度、媒体准备完成预览、替换、清除和失败提示。
- 底部胶囊固定顺序：目标语言、翻译方式、口播音色、去文案；右侧按钮固定为「开始口播翻译」。
...
- 默认视口和 820px 窄屏。
- 页面主体只展示项目与结果，不出现独立配置大卡片。
- 大输入区点击/拖拽上传、上传进度、准备后视频预览、替换和清除。
- 底部四个胶囊参数、去文案区域编辑浮层、右侧提交按钮和确认层。
- 宽屏优先单行、窄屏自然换行，参数和按钮不得被裁切或形成横向滚动。
- 已有项目带入后使用同一 Composer，不跳转到另一套页面样式。
- 所有处理阶段的真实进度。
...
- 口播翻译创建入口与现有视频生成保持同一 Composer 形式；页面主体不再承担配置。
- 大输入区只用于视频上传、处理进度和预览，不新增 prompt 文本字段。
- 目标语言、翻译方式、口播音色和去文案都在底部胶囊参数栏，右侧提交按钮位置与现有视频生成一致。
