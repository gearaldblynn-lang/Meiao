...
export const buildJobRuntimeLogMeta = ({
  ...
  return {
    ...
    transportErrorCode: String(error?.transportErrorCode || '').trim(),
    transportErrorSyscall: String(error?.transportErrorSyscall || '').trim(),
    transportRemoteAddress: String(error?.transportRemoteAddress || '').trim(),
    transportRemotePort: normalizeLogNumber(error?.transportRemotePort, 0),
