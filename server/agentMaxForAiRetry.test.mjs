...
test('Agent Center never retries a MaxForAI paid image request automatically', () => {
  ...
  assert.equal(getImageGenerateTransientMaxRetriesForModel('maxforai-image-2-standard', env), 0);
  assert.equal(getImageGenerateTransientMaxRetriesForModel('Image-2高', env), 0);
