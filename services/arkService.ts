...
export const analyzeOneClickReferenceSet = async (
  ...
  try {
    ...
    if (validReferenceUrls.length === 0) {
      return { status: 'error', description: '', message: '设计参考图尚未上传完成，请等待图片上传成功后再分析。' };
    }
...
- 排版：主体居中偏大，标题区上置，卖点区右下角补充`;
    ...
    if (safeLogoUrl) {
      inputContent.push({ type: "text", text: `[品牌logo图] ${safeLogoUrl} 是品牌logo图，仅用于识别我方品牌，不要把其他品牌logo当作我方品牌元素。` });
...
[渲染风格定义] clean premium studio lighting`;
    ...
    const userPrompt = `分析原图。${safeReferenceUrl ? `参考图。` : ""}\n${modeSpecificPrompt}\n请用简洁专业的英文输出。`;
...
8. 不得把文案信息混入“画面描述”栏位；若上游输入是旧格式，必须先改写成标准格式再输出。
    9. ${safeLogoUrl ? '品牌logo图仅用于识别和还原我方品牌logo，不得带入任何竞品logo或他牌标识。' : '若产品素材中出现竞品logo或他牌标识，最终方案与生成图都必须去除这些非我方品牌标识。'}
...
${referenceAnalysisSummary ? `【参考分析结论】\n${referenceAnalysisSummary}` : safeStyleUrl ? `风格参考图：${safeStyleUrl}。仅提炼其配色、光影、材质与氛围作为整套视觉基调。` : ""}
${safeLogoUrl ? `品牌logo图：${safeLogoUrl}。该图仅用于识别和还原我方品牌logo，不得把产品素材图或设计参考图中的其他品牌logo带入最终画面。若产品素材中出现竞品logo或他牌标识，最终生成图必须去除或替换为品牌logo图对应的我方logo。` : ""}
...
请开始策划。`;
    ...
    if (safeLogoUrl) {
      inputContent.push({ type: "text", text: `[品牌logo图] ${safeLogoUrl} 是品牌logo图` });
      inputContent.push({ type: "image_url", image_url: { url: safeLogoUrl } });
    }
...
export const generateFirstImageReplicationSchemes = async (
  ...
  try {
    ...
    const settledResults = await Promise.allSettled(validReferenceUrls.map(async (referenceUrl, index) => {
      ...
      const safeReferenceUrl = safeReferenceUrls[index];
...
请先抽取复刻主图参考${index + 1}的真实版式，再把卖点替换到参考图原有海报/页面文案位。产品素材只决定替换进去的商品本体，商品区关系必须来自复刻主图参考${index + 1}；我方产品包装文字和标签按素材原样保留，不作为文案替换对象。`;
      ...
      inputContent.push({ type: 'text', text: `[复刻主图参考${index + 1}] 图片URL：${safeReferenceUrl}。这是唯一版式、风格、信息层级参考，必须基于这张图做主图裂变。` });
      inputContent.push({ type: 'image_url', image_url: { url: safeReferenceUrl } });
      ...
      return { referenceUrl: safeReferenceUrl, scheme, status: 'success' as const };
    }));
...
E Example 示例
{
  ...
  "evaluation": "..."
    ...
    const refInstruction = safeReferenceUrl
