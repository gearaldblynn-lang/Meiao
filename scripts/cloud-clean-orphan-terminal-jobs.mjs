...
const backupRoot = backupRootArg
  ...
  : '/www/backup/meiao-orphan-terminal-jobs';
...
const TERMINAL_EMPTY_STATUSES = new Set(['failed', 'error', 'cancelled', 'interrupted']);
...
const collectProjects = (state = {}) => {
  ...
  ONE_CLICK_BRANCH_KEYS.forEach((key) => {
    if (Array.isArray(oneClickMemory[key]?.projects)) projects.push(...oneClickMemory[key].projects);
  });
...
const isOrphanTerminalJob = (job, stateKeys) => {
  if (cleanId(job.module) !== 'one_click') return false;
  if (!TERMINAL_EMPTY_STATUSES.has(cleanId(job.status))) return false;
...
const [jobRows] = await pool.query(
  `SELECT *
   ...
   WHERE module = 'one_click'
     AND status IN ('failed', 'error', 'cancelled', 'interrupted')
...
for (const row of stateRows) {
  ...
  try {
    ...
    reports.push({ userId: row.user_id, username: row.username, error: 'invalid state json' });
    continue;
  }

  const stateKeys = collectStateJobKeys(state);
  const orphanJobs = (jobsByUser.get(row.user_id) || []).filter((job) => isOrphanTerminalJob(job, stateKeys));
...
console.log(JSON.stringify({
  ...
  scannedTerminalJobs: jobRows.length,
  totalRemovedJobs,
