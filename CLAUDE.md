...
## 3. 已诊断根因库 ★(持续维护,截至 2026-08-11 已记录至 #117)
...
- **#117 ✅ 已修并待云上验收(2026-08-11)· KIE 已成功的分镜图被前端超时误判且 durable job 无法自愈**
  根因:浏览器后台休眠可使轮询定时器跨过前端 deadline；超时 catch 虽会最后一次查 `internal_jobs`，却不识别该次查询已经是 `succeeded`，只要有 providerTaskId 就返回“结果待同步”。分镜板生图又没有显式传 `jobModule=video`，脱离模块上下文时任务落为 `module=unknown`；公共 hydration 只接受 `video`，因此后端真实成功也无法修复页面卡片。云上案例 `fe0e3ba1d15d3532d19acf73` 145 秒成功，前端却在提交后约 30 分钟写入待同步；同日 2/2 个分镜生图 job 都是 `unknown`。
  修复:`src/services/kieJobResult.mjs` 统一终态 job 到 KIE UI 结果的映射，正常轮询与超时后最后复查共用；`videoStoryboardService.ts` 对所有分镜板生图显式写入 `jobModule: 'video'`；`shellDataAdapter.ts` 只对同时满足 `unknown + kie_image + storyboard + taskPurpose/planningPurpose + shellProjectId + boardId===shellBoardId` 的历史任务做兼容恢复，不将其他 unknown job 路由进视频。
  如何避免:**任何长任务的 timeout 处理都必须把“最后一次 durable job 复查”当成权威终态，禁止仅凭 deadline/providerTaskId 降级为 generating。所有跨异步边界的付费任务必须显式携带 module + workflow purpose + project + result/board 四层耐久身份，不得依赖 React/全局上下文。回归必须覆盖后台休眠后终态复查、历史 unknown 正例和身份不完整负例。**

- **#116 ✅ 已修并完成云端运行时收口(2026-08-10)· 信息型 readiness 被当成发布门禁，`ready=false` 仍切换生产版本**
  根因:`probe:voiceover-translation -- --readiness` 为了 disabled-first 巡检始终退出 0，发布脚本却直接将其当作强制门禁；所以 Demucs/Whisper 缺失时仍可以 reload，页面代码已上线但 health 持续 `ready=false`。PM2/Node 子进程当时还以 root 运行，降权后又暴露日志目录、部署 marker 和 COS readiness 文件三个 root-only 隐性依赖，导致新子进程启动或 health 失败。
  修复:`scripts/probe-voiceover-translation.mjs` 新增仅 readiness 可用的 `--require-ready`，false 时返回 2 且仍只输出有界布尔摘要；口播与 COS 发布探针均以专用 `meiao-app` 账号运行。`ecosystem.config.cjs` 将 PM2 子进程降权，日志改到 `/var/log/meiao`；`.env.server` 为 root 所有+服务组只读，`server/data` 和 COS readiness 由服务账号持有，部署 marker 保持 root 所有但为应用只读的 `0644`，模型运行时为 root 所有的只读目录。最终标准发布 `meiao-20260810-voiceover-final-511f61a` 全自动通过，不需要人工处理门禁。
  如何避免:**运维命令必须区分“打印状态”与“强制通过”；任何布尔 readiness 进入发布链，测试都必须证明 false 会在 drain/reload 前中止。降权不能只检查主源码目录；日志、密钥、可变数据、部署门禁、真探针状态和本地模型都必须用真实子进程身份验收，不得用 root 成功代替应用账号成功。**

- **#113 ✅ 已修待部署(2026-08-04)· 虚拟模特的后台管理状态与公共可选状态漂移**
  根因:后台列表只要发现任意 `draft` 版本就优先显示“草稿”，而公共模特列表和新任务快照仍只检查 `virtual_models.status/current_version_id` 指向的旧已发布版本。因此同一模特会在管理页显示草稿，却继续出现在公共选择器；弹窗又信任接口返回，旧选择只在提交链路中才暴露失效。
  修复:`listPublishedVirtualModels` 和新任务 `createVirtualModelGenerationJobSnapshot` 统一为“当前版本已发布且不存在活动草稿”；发布新版本时将同模特其他草稿/发布版统一收敛为 `unpublished`；选择器对服务端响应再做一次已发布校验，旧选择失效时立即清空并显示中文原因。历史任务继续按 `publishedAt + selectedAssetIds` 重放，不被新任务门禁影响。
  如何避免:**“管理页状态”“公共可见”“新任务可用”必须共用同一版本生命周期判据；只改显示标签或只在任务尾端报错都会留下窗口期。回归必须同时覆盖列表、详情、新任务快照、发布收敛和历史重放。**

- **#114 ✅ 已部署(2026-08-05)· Logo 框选是语义定位，终态保护必须区分物体融合与二维角标**
  根因:全图生成模型没有像素级选区边界，仅靠“只修改 targetRegion”会复制 Logo、改字或改动未绑定标识；但把用户大致框直接升级成硬裁切边界、再对所有场景精确叠加上传 Logo 也是过度修正：框不精确时会切坏融合，产品/衣物/包装等真实表面上的 Logo 需要服从透视、曲率、材质、光线与遮挡，不能按二维图稿硬贴。云上任务 `58338f7e9ef15dee21444a87` 的越界截图发生在 v6 发布前，证明原始越界问题真实存在；用户对 v6 合同的复核又暴露了硬边界与统一精确叠加的设计缺陷。
  修复:策划升为 v5，把框选作为 `semanticSelection`，在原图中识别真实 `targetBounds`，并分类 `surface_integrated`/`graphic_overlay`。程序从真实目标生成可配置外扩的 `editEnvelope`，只接受包络内 provider 候选并在边缘羽化，包络外恢复原图，所以未绑定 Logo、文字和产品细节不随整图生成被改写。物体表面保留 AI 融合结果，不再终态硬贴；二维角标才按 `containedBounds` 精确合成透明身份参考。终态回写 `logoReplaceSemanticBlendGuarded=true`；历史 v4 分析可只重做本地终态保护，不重新调用付费 provider。commit `1e89705`，release `meiao-20260805095735-1e897059fcce`。
  如何避免:**用户选框、真实语义目标、允许编辑包络是三个不同概念；局部编辑工具不得把粗框当像素真值。Logo 身份保护必须先判断承载方式：真实表面由 AI 做视觉融合，独立二维图层才做精确像素合成；无论哪种方式，未绑定内容与编辑包络外像素由程序恢复，不再依赖提示词自律。**

- **#115 ✅ 本地已修待验证(2026-08-05)· 产品策划完全不描述视觉身份会丢失具体结构注意力**
  根因:#111 正确确立“上传产品图是视觉身份最高真值”，但 v6 又把它过度实现为策划只输出位置、透视、材质互动、遮挡和接触阴影，并明确禁止产品结构与材质描述。面对多角度素材、旧产品轮廓诱导、遮挡和目标视角变化时，生图模型没有一份当前产品专属的组件拓扑、接口边缘、刚柔属性与必须露出结构作为注意力锚点，容易把具体产品简化成同类通用产品。固定的“五维保持一致”只能表达优先级，不能指出本产品到底有哪些不能错的结构。
  修复:组合产品策划升为 v7，每张参考图独立分析。逐产品从绑定多角度图提取有证据的实体边界、轮廓比例、组件拓扑、接口边缘、材质工艺、固有色、图案文字、刚柔属性、关键细节与禁止变化；逐区域结合当前参考图输出必须可见结构、几何适配、受光色彩、材质互动、遮挡、接触阴影和旧产品清除。解析器严格校验绑定和必填字段，发现关键角度证据不足时在生图前停止；字段、数组、结构化策划整体和最终 prompt 均有 env 可调的保守上限。生图只收到一份去 P 标记、去 URL 的结构化执行合同，原始产品图仍是最高真值，策划文字不允许发明或覆盖图片事实。历史 v1-v6 只用于恢复。
  如何避免:**多模态身份链路要同时满足两点：原图是唯一视觉真值，策划提供有证据的产品专属注意力锚点。不能用长篇同义复述重建第二份身份，也不能删到只剩通用“保持一致”；新增字段必须进入单一执行合同、受结构和总长双重约束，并用复杂多产品回放验证具体拓扑只出现一次。**
...
- **#67 ✅ 已修(2026-07-15)· `localAssetId` 被当成云端素材身份拒绝全部状态写入，首图成功结果失去项目锚点且新卡沉底**
...
- **#75 ✅ 已发布·观察中(2026-07-17)· 产品还原结构化子功能在恢复层被默认改成原图精修，重试又拆成重复项目卡**
...
- **#76 ✅ 已发布(2026-07-21)· 原创分镜在模型提交前调用未导入的 `getSegmentLabel`，所有结果卡同步失败**
  ...
  修复:把已有 `getSegmentLabel` 作为 `videoStoryboardPlanning.ts` 的共享命名导出，并由 `videoStoryboardService.ts` 显式导入，保持分段命名只有一个实现。新增回归测试同时执行共享函数并锁定服务文件必须存在运行时 import，先复现导出为 `undefined`，修复后验证 `getSegmentLabel(0) === '分段一'` 且 import 合同成立。发布前等待 3 个已提交 KIE 图片任务自然结束，未使用活跃任务 override；发布后公网/宿主机 health、Temporal worker、COS 真探针、前端 `index → ShellMigratedApp → videoStoryboardService` 资源链和三份关键源码 SHA-256 均通过。
...
- **#77 ✅ 已发布(2026-07-21)· 已兼容 MP3/H.264 仍被强制转码，上传阶段又长时间没有真实进度**
...
- **#79 ✅ 已发布并完成零 502 实发验收(2026-07-23)· 单上游先停后启发布必然制造官网 502**
  ...
  修复:正式 PM2 合同改为单实例 `cluster + wait_ready`，完成 bootstrap/listen 后才发 ready，旧进程收到信号后优雅停 worker、Temporal、HTTP 和连接池。发布脚本先用 marker 暂停新写入和 worker，等待活动写请求/任务归零。经典 MySQL worker 与 Temporal activity 在 claim 前必须获取同一 MySQL 命名锁并重查 marker；部署用该锁做最终零任务屏障，从而关闭“worker 先检查、排队等锁、部署释锁后再抢跑 claim”的付费竞态。释放锁后原子切 dist 并 `pm2 startOrReload`。只有精确 release health 通过才移除 marker；若仅旧内存进程健康，因磁盘源码/依赖已属于未验证 release，失败清理必须保留永不过期 `manual` marker 与 mutex，禁止新写入，不能假报完整回滚。旧 fork 首次迁移已通过只监听 `127.0.0.1:3101` 的候选、Nginx 双切换和表锁内停旧进程完成；随后用标准脚本真实执行第二次发布，正式进程保持 `cluster_mode` 并切到 release `meiao-zero502-standard-844605f95229`。发布后绕过本机代理连续直连官网首页与 health 共 2000 次，HTTP 502、网络错误和其他状态码均为 0；marker/mutex 均清除，3101 候选已删除，四个关键运行文件云上 SHA-256 与本地一致。
...
- **#80 ✅ 已修并完成云上实图验收(2026-07-23)· 任务生成成功但本地结果图经 Nginx X-Accel 返回 403**
...
- **#81 ✅ 本地已修、未部署(2026-08-03)· 组合产品替换按左右顺序猜位置，跨参考图映射不稳定**
  根因:旧组合替换合同把产品组按首次出现顺序映射到参考图中“从左到右、从上到下”的区域，但多张参考图的产品布局、遮挡和前后关系并不一致，也没有逐参考图持久化的位置真值；工作流直接把整批产品素材和参考图交给生图模型，没有先建立可校验的区域绑定。
  修复:组合模式要求每张参考图分别手工标记 P1 到 Pn，并在任何付费提交前验证所有参考图完整覆盖全部产品组。随后每张参考图独立生成 P 标记图，以“参考图、标记图、按绑定编号排列的产品素材”调用策划模型；严格解析器拒绝交换、遗漏、重复或新增绑定。生图只接收干净参考图和产品素材，以策划结果加归一化数值区域执行，禁止把标记图、P 编号和框线送入生成输入。单品替换继续直接生成，且本轮不增加生成后质量验收。
  如何避免:**多对象替换不得把视觉顺序或模型猜测当身份合同；位置真值必须逐参考图、结构化、可持久化，并在策划和生成边界使用同一图片编号映射。所有批次先全量预校验再创建任一付费任务，避免半批提交后才发现未标记素材。**

- **#82 ✅ 本地已修、未部署(2026-08-03)· AI 原生 Logo 替换成功后仍返回 provider 临时尺寸和临时 URL**
  根因:新工作流已经让生图模型一次性完成 Logo 清理、替换和融合，但成功结果直接沿用 provider 返回值；真实验收中原图为 800×800，provider 输出为 1254×1254 临时 URL。比例虽一致，结果仍未恢复原画布尺寸、未转存站内受管资产，刷新后 backend job 也可能重新覆盖为临时来源。
  修复:新增整图 finalizer，下载完整 provider Blob，先校验输出与原图比例漂移不超过 2%，再把整张结果等比归一化到素材快照的原始宽高并上传为站内受管结果；最终 URL 回写原 backend job，同时保留 `originalProviderImageUrl`。该处理只做整图尺寸归一化，不截区、不生成遮罩、不清理或后贴 Logo。
  如何避免:**任何 AI 整图编辑的 provider success 都不是最终发布合同；必须继续验证真实字节比例和尺寸、转存受管资产并回写 job 真相源。整图尺寸归一化与局部内容合成要严格分开，禁止借后处理重新引入 Logo 硬贴。**

- **#83 ✅ 本地已修、未部署(2026-08-03)· 组合产品标记编辑器无限重置且受保护素材破图**
  根因:未标记参考图没有 `initialRegions` 时，组件参数内联默认值 `=[]` 每次渲染都会创建新引用；初始化 effect 写本地 state 后依赖再次变化，形成 `Maximum update depth exceeded`，拖拽结果持续被默认区域覆盖。另一个独立缺口是编辑器用原生 `<img>` 直接读取上传后返回的 `http://127.0.0.1:3100/api/assets/file/...`；页面运行于 `localhost:3001`，浏览器把同一后端的不同主机名视为跨源，下载代理又无法代替当前用户读取 owner 保护素材，画布和产品缩略图因此破图。
  修复:空区域默认值提升为模块级稳定常量，阻断初始化 effect 自激循环；编辑器预览统一先用 `resolvePublicAssetUrl` 把本机 managed asset 归一为同源路径，再由 `fetchImageBlobWithProxy` 携带会话 Bearer 获取 Blob、转换 object URL，并在切图或卸载时回收。真实浏览器回归确认两张参考图各自 `0/2 → 2/2 → 已标记`、重新打开仍恢复区域，主图与 P1/P2 缩略图可见且控制台零错误。
  如何避免:**React effect 依赖不得使用参数内联 `=[]/{}` 这类每次渲染重建的默认引用；会写 state 的初始化 effect 必须用稳定输入做浏览器交互回归。任何 owner 保护素材不得直接交给原生 `<img>`，本机绝对 managed URL 先归一为当前页面同源路径，再走 Bearer fetch → Blob → object URL，并验证真实像素可见而不是只看 DOM 中存在 `<img>`。**

- **#84 ✅ 本地已修、后置审查策略已由 #89 撤销、未部署(2026-08-03)· Logo 品牌可识别但内部排布被生图模型重组**
  根因:旧 AI 原生 Logo 替换只把字形、颜色、比例和笼统的 `element arrangement` 写进提示词，没有把 Logo 内部元素顺序、相对位置、对齐、组合方向、可见图稿比例和选框几何做成共同执行的合同。真实任务虽然升级了 v2 身份分析，生图提示词仍没有数值框位、目标框比例和整组 contain 边界；模型把“图形在上、主标居中、副标最下方”重排为“图形在左、文字在右”。质检只看 800×800 整图，两处 Logo 约 63×55 像素，模型误判全部通过。进一步生成三联放大证据后确认验收框本身也偏移：编辑器中原图按 contain 显示为正方形，但坐标层仍占满更宽容器，视觉拖拽与保存比例不在同一坐标系，R1 主要落在旧标右侧空白，R2 只部分覆盖旧标；旧分析又没有验证选框是否真正完整包住旧 Logo。修正框位后的下一次真实任务虽保持了纵向排布，却暴露第二个几何语义错误：`identityReferenceAspectRatio` 来自加 6% 安全边距后的导出裁剪框，而不是 Logo 真正可见内容边界，导致身份比例从 NOVA LEAF 的 1.3802 被稀释成 1.3257。恢复侧还有独立缺口：同一 Logo 生图 job 的后置质检失败已进入 jobs hydration，但共享 `taskResultReconcile` 仍执行“已有完成媒体必胜”，旧 `completed` 因而覆盖 incoming `error` 并继续写回 `app_state`，刷新首屏短暂谎报完成。
  修复:Logo 输入继续使用只裁外围空白且保留有意底板的紧边界身份参考，但裁剪器现在同时返回“可见内容紧边界”和“防裁切安全边界”：上传图片使用安全边界，身份可见比例只使用前者。编辑器坐标层宽度改为与 contain 原图的实际宽高比共同计算，视觉框与持久比例保持同一像素平面；界面明确选框既要覆盖旧 Logo，也是新 Logo 整组最终允许占用范围。新分析合同升级为 v3，逐区强制输出并校验 `selectionContainsOldLogo/selectionCoverage` 和 `logoIdentity`；框偏移、只覆盖局部或框错对象时在付费生图前失败，历史 v2 只允许恢复读取。生图提示词新增归一化框位、目标框比例、身份参考比例、预期 layout 和整组 contain 边界，明确目标框比例不得反向决定 Logo 比例。曾实现的出图后放大质检及“审查失败覆盖成功图”策略已按用户产品决策由 #89 完整撤销；当前正确性边界收回到生图前分析合同，成图由用户直接验收。
  如何避免:**Logo 身份不能只用“可识别”或笼统提示词兜底；选框覆盖、内部拓扑、可见比例、数值几何、固定映射和生图提示词必须形成同一前置合同。目标框比例永远不能决定内部排布。真实验收必须核对参考图、标记图、最终图、页面冷刷新终态和任务账本；不要再增加出图后 AI 裁判来吞掉用户满意的可见结果。**

- **#85 ✅ 本地已修、历史任务真实恢复通过、未部署(2026-08-03)· 组合产品标记泄漏到生图且冷刷新重复结果**
  根因:旧组合链把 `[干净参考图, P1/P2 标点图, 产品素材]` 一起交给生图模型，标点图不仅可能把编号和框线带进结果，也让模型在产品身份与定位提示之间自行取舍；策划 v1 又只描述位置、尺度、光影和遮挡，没有强制提取产品轮廓、结构比例、配件、材质、Logo、标签和包装文字等身份。恢复侧还有独立缺口：成功任务实际走“万物替换批次聚合”分支，前一版恢复补丁却只改了另一个通用分支；新增的 `batchCount`、策划积分和生图积分字段在持久化读取时又被丢弃。同一成功 batch 因而同时命中旧失败批次身份和已有 provider/job 身份，旧失败被替换后仍留下另一份成功结果，冷刷新显示 `3/3`、重复图片和重复积分。
  修复:策划合同升级为 v3，每张参考图独立提交“干净参考图 + 当前标点图 + 按 P 编号排列的产品素材”，严格输出并校验 `products[]` 产品实体边界、不可带入的非产品参考元素、精确视觉锚点、`regions[]` 数值位置和一一绑定；新任务拒绝 v1/v2，历史恢复仅在显式兼容开关下只读旧合同。生图输入改为“干净参考图 + 产品素材 + 可选 Logo”，标点图只留在策划边界，提示词用数值区域合同表达位置并递归清除 P 编号文本。本地恢复在真实批次聚合分支组合 `策划积分 + 生图积分`，按 batch 去重，持久化完整保留批次数和两段积分；即使 jobs 尚未同步，首屏也用权威 `batchCount` 清理同批旧失败项并归一到 `completed/2/2/13.12`。真实项目 `proj-1785695421638` 只查询原有 2 个策划和 2 个生图任务，未重提付费任务；冷刷新首屏显示 2 个结果，两张受保护图片均解码为 1264×848。
  如何避免:**策划用标点图，生图只能用干净参考图和结构化位置合同；多产品身份必须成为严格可解析字段，不能只靠自然语言“尽量还原”。新增任何批次身份或计费字段时，要同时覆盖创建结果、job 聚合、持久化读取、无 jobs 首屏和 jobs hydration 五个边界，并用“已有成功结果 + 同 batch 历史失败 + 当前 jobs”的真实形态锁定冷刷新。自动质量验收是否启用是独立产品决策，不得与输入合同和恢复正确性混在一起。**

- **#86 ✅ 本地已修并完成真实双图验收、未部署(2026-08-03)· 组合产品策划成功却被解析器拒绝，单图重试又会重跑整批**
  根因:真实策划 provider 返回的是“唯一完整 JSON + 末行 `final_answer`”，旧解析器却强制要求同时出现 `commentary/final_answer`，两条已成功且已计费的策划因此被前端当成结构无效。首次生图失败后，项目又被 hydration 标为 `sourceType=job`，通用后端重试分支在组合替换专用恢复之前抢跑；专用恢复本身还会重跑所有参考图，即使用户只点某一条失败结果。
  修复:严格解析器新增已确认的 `JSON + final_answer` 包装，仅当末行唯一且前文是单个完整 JSON 时接受，任何额外文字、第二个 JSON 或尾随内容继续 fail closed。组合替换恢复在通用 job retry 之前取得优先级；用户点击具体结果时，只取该原始 batch 的参考图和已成功策划 ID，保留其他成功结果，并把原始 batch index/count 穿透到 prompt、job 和持久化。真实项目 `proj-1785724529187` 始终只有 2 条策划；第 1 张被 provider 拒绝后的恢复只新建 1 条生图，第 2 张没有重提。最终页面为 `completed/2/2/13.07`，两张分别满足“红泵左绿罐右”和“绿罐左红泵右”，无 P 编号、标记框或说明文字。
  如何避免:**结构化 provider 解析必须保留真实原始响应形态做回放样本，不能只用理想化 fixture。“一图一任务”必须同时适用于新建和重试：点单条失败结果只能复用该 batch 的策划并新建该 batch 的生图，不得用通用 retry 重放旧 payload，也不得重跑已成功图。**

- **#87 ✅ 本地已修并完成真实单次恢复验收、后置审查策略已由 #89 撤销、未部署(2026-08-03)· Logo 多阶段恢复被通用 job 重试抢跑，提示词超限后又会卡生成中并重复结果**
  根因:Logo v3 的分析、生图和整图归一化没有贯穿恢复身份。分析回复的真实包装是“唯一 JSON + 末行 `final_answer`”；生图又把完整分析无界序列化，首次恢复生成 21685 字符并在 provider 创建前被文本上限拒绝。点“重生成”后，`sourceType=job` 通用分支先调 `retryInternalJob(resultId)`，返回“任务不存在”，Logo 专用的“复用成功分析、新建生图”分支无法到达。恢复成功后，jobs hydration 又按 backend/provider ID 合并而不认 batch，同一参考图的旧失败与新尝试被算成两个结果。旧后置质量门控还会把生成状态错误改写，现已由 #89 删除。
  修复:严格解析器接受经回放确认的单 JSON 加唯一末行 `final_answer`，其他尾随内容继续拒绝。生图只投影执行必需的分析字段，用 `VITE_LOGO_REPLACE_GENERATION_PROMPT_MAX_CHARS` 在付费提交前守住默认 18000 字符上限；真实重放从 21685 降至 15021。Logo 恢复显式跳过通用 backend retry，按项目与 batch 查找已成功分析 ID，缺任一批即停止，不重复分析计费。Logo 结果合并改为同 batch 最新尝试权威覆盖，并用 job payload 的 `batchCount` 修复历史虚高任务数。最终状态现在只尊重分析、生图与资产处理，历史质量字段不再参与 hydration。
  如何避免:**多阶段付费流程必须在创建、恢复、解析、提示词容量、batch 去重和计费展示边界共用同一阶段身份。可恢复阶段必须复用已成功付费上游，不得用通用 job retry 重放旧 payload；同 batch 的不同尝试不得同时成为用户结果。真实验收必须核对页面冷刷新、job 账本、prompt 长度、复用 ID 和最终图。**

- **#88 ✅ 本地已修、未部署(2026-08-03)· 产品身份字段存在但在生图提示词中被稀释，材质、颜色、图案和结构仍会漂移**
  根因:组合替换 v3 策划已经采集轮廓、结构、材质、颜色和细节，但最终生图只把这些字段埋在包含构图、光影、遮挡等内容的大型策划 JSON 中，再补一条泛化的“保持产品一致”。其中颜色与图案还共用一个字段，模型在构图适配和自然融合时容易把产品固有属性降级为软参考，出现同类通用化、改色、改材质、删小组件或重绘图案。
  修复:新任务策划合同升级为 v4，每个产品必须分别输出 `identityLock.materials/details/colors/patterns/structure` 和非空 `forbiddenChanges`；缺任一维度直接拒绝策划结果，不进入付费生图。最终提示词在普通策划数据之前前置逐目标区域、逐产品输入图绑定的 `<product_identity_lock_contract>`，明确环境光只能影响高光、阴影与反射，透视只能影响二维投影，不能改固有色、材质种类、表面工艺、图案、轮廓比例、组件几何和装配关系。历史 v1-v3 仅允许恢复读取，并从已有身份字段推导同一五维锁定；本轮仍不增加生成后质量验收模型。
  如何避免:**产品身份关键属性不能只“被分析到”，还必须在最终执行提示词中成为独立、靠前、逐产品绑定、可严格解析的硬合同。固有颜色与环境光色、真实结构与透视投影、产品图案与参考图装饰必须分开建模；任何新合同字段都要同时覆盖新建解析、最终提示词、历史恢复和缺字段 fail-closed 测试。**

- **#89 ✅ 本地已修并完成历史结果恢复回归、未部署(2026-08-03)· Logo 生图成功却被出图后 AI 审查改成失败**
  根因:Logo 工作流在成功生图和最终资产转存后又自动创建 `logo_replace_quality_check`，用第二个多模态模型评判结构和微小文字；审查结论既额外计费，又通过 `logoReplaceQualityStatus` 覆盖成功 generation job。data adapter 把缺审查视为生成中、审查拒绝视为失败，共享结果对账还专门让“同一 Logo job 的审查失败”压过已有完成媒体。因此项目已有可见成图，页面仍显示“失败原因”，冷刷新也无法自愈。
  修复:运行时 processing mode 升级为 `ai_native_analysis_generation_v4`。Logo 生命周期只执行分析、生图、整图比例/尺寸归一化和受管资产转存；生图及资产处理成功即返回完成，不再生成审查证据、创建审查 job 或累计审查积分。hydration 完全忽略历史 `logoReplaceQualityStatus/summary/errorCode`，只按真实 generation job 状态映射；结果对账恢复“完成媒体优先”并清除历史错误字段，使已有成功图无需重新生图即可从审查失败恢复为完成。持久化首屏也按 Logo batch 和真实 `createdAt` 只保留最新尝试，避免在 jobs hydrate 前短暂显示“旧失败 + 新成功”两条结果。真实 provider 失败仍保留错误。
  如何避免:**用户可见生成结果不能由另一个主观模型自动裁判并覆盖，尤其不能让“文字太小无法辨认”把成功图片改成任务失败。正确性应前置到选框覆盖、素材绑定、结构分析、数值几何和生成提示词；出图后由用户直接验收。回归必须同时覆盖“未来不创建 quality job”“历史 quality failed + generation succeeded 恢复完成”“真实 provider failed 仍失败”“冷刷新不再出现审查失败原因”。**

- **#90 ✅ 本地已修并完成真实页面回归、未部署(2026-08-03)· 组合产品自动分组在删除、恢复或续传后碰撞，两个不同产品同时显示 P1**
  根因:自动分组直接使用当前数组序号生成 `product-group-N`。素材删除、重排、草稿恢复或继续上传后，仍存活素材的旧组号可能与新素材按当前位置生成的组号碰撞；下拉框选择尚不存在的 P2 时又可能回退复用碰撞 ID，所以视觉和底层数据都会把两个独立产品误认成同一组。
  修复:自动组 ID 改为基于稳定素材身份生成，并持久化 `productGroupAssignment=auto|manual` 区分系统分配与用户主动合组。进入组合模式时自动修复历史的非手动重复组号；用户手动选择已有组仍允许多角度素材归为同一产品，选择新编号则创建真正唯一的手动组 ID。真实登录页面已确认两张独立产品素材分别显示 P1、P2，草稿归一化和恢复同时保留组号来源。
  如何避免:**业务身份不得由可变数组序号派生。自动身份必须稳定且唯一，任何允许“重复即合组”的功能都要显式记录重复来自用户操作还是历史碰撞；删除、重排、恢复、续传和选择不存在的新槽位必须进入回归矩阵。**

- **#91 ✅ 本地已修并完成真实结果对照、未部署(2026-08-03)· 产品固有色虽被列为身份字段，暗场生成仍把中灰产品压成接近黑色**
  根因:v4 只用一个自然语言 `colors` 字段描述固有色，缺少逐组件颜色地图、相对明度/饱和关系和中间调白平衡边界；最终提示词一边禁止改色，一边又允许“根据场景调整整体明暗”，使场景暗调、统一色温、LUT 或曝光可以覆盖产品主体中间调。真实项目 7 对照中，产品原图的帽子为中性中灰，成图在暗场景中被整体压成接近黑色，灰阶层级已发生实质变化。页面草稿还残留旧任务的“红色泵瓶/绿色圆罐”要求，与当前帽子、面罩、冰袖素材冲突；旧合同没有明确要求忽略这类过期产品颜色描述。
  修复:组合策划升级为 v5，每个产品除五维身份锁外必须输出 `colorPreservation.componentColorMap/relativeColorRelationships/midtoneAndWhiteBalanceRule/forbiddenColorShifts`，缺一项即在生图前 fail closed。最终提示词新增前置 `<product_color_fidelity_contract>`，明确禁止对产品区域应用全局 LUT、滤镜、统一色温、统一饱和度、统一曝光或整体压暗；场景光只允许产生局部高光、局部阴影和局部反射，产品主体中间调必须保持同一色相家族、明度层级和相对饱和度。策划与生图同时规定用户补充要求不得重新定义产品颜色、材质、结构、组件、数量或映射，与当前产品素材冲突的旧描述必须忽略。历史 v2-v4 恢复在存在产品颜色字段时推导同款结构化保护；字段不足的 v1 和单品模式至少继承同一组全局颜色保护约束。本轮仍不增加生成后质量验收模型。
  如何避免:**“禁止改色”不能与“允许整体明暗/统一调色”同时存在。颜色身份必须拆成逐组件固有色、相对关系和允许的局部受光范围，并明确把产品蒙版排除在场景全局调色之外；暗场验收必须检查中间调与灰阶层级，不能只看色相名称。**

- **#92 ✅ 本地数据已恢复并完成真实页面验收、未部署(2026-08-03)· 临时 worktree 验收数据与主项目同名 admin 分叉**
...
- **#95 ✅ 本地已修、待部署(2026-07-28)· 口播翻译按整段慢放，保留原背景又把旧口播残留和分离伪影带入成品**
  ...
  修复:每个检测到的口播时间段独立 TTS；长语音在默认上限 `1.75` 内加速，短语音允许在默认下限 `0.75` 内安全减速，仍短于窗口时把剩余静音均分到语音前后，避免全部左对齐。译文密度默认收紧为 24 bytes/s，并提示空格分词语言不超过约 3 词/秒。Demucs 仍只用于抽取旧口播供识别；最终 MP4 只映射原视频画面和对齐后的新口播，完全不输入原音轨、`no_vocals` 或环境声，背景音乐留给后期重新添加。KIE `waiting/queuing/generating` 都按同一 task ID 有界轮询；分析校验失败可在用户确认后递增 analysis attempt，旧合并检查点与新时间窗不一致时在任何付费调用前 fail closed。
  如何避免:**口播对齐必须以可持久化的分段时间窗为最小单位，不能假设多 turn TTS 会返回内部时间戳；禁止把多个分段合并后整体慢放，但单段可以在配置的可理解下限内减速，并把无法填满的静音对称放置。若产品合同是“纯新口播”，最终混音的 FFmpeg 输入合同只能包含底片视频和新口播，不能靠声源分离估计背景；真实频谱回归必须证明原音乐频率、旧人声频率均消失而新口播存在。异步 provider 的统一任务状态表优先于单模型示例，所有非终态都要回归；真实 canary 必须逐组核对 `actualDurationMs/atempo`、窗口内实际起止位置、最终时长和 Range 资源链。**

- **#96 ✅ 全局修复已发布并完成故障账号资源链验收(2026-07-29)· 受保护素材在项目卡、上传栏和大图预览反复破图**
  根因:公共模特素材加固把 owner/access-key 校验前移到所有 `/api/assets/file` 分支，但浏览器凭证保存在 localStorage，原生 `<img>` 不会携带 Bearer。前两轮只修了 `ProjectCard` / `PlanEditor`，没有做运行时图片消费者的全仓库审计；`MaterialPreviewBar`、`ImageLightbox`、Agent Center、OneClick、BuyerShow、Retouch、Video 等 33 个组件仍有 78 处原生 `<img>`，所以同一根因会换入口复发。董丹丹本轮两张风格参考图的 owner、active 状态、MIME、字节和 SHA-256 均正确，匿名请求 403、owner 鉴权请求 200，排除了文件损坏、数据库丢失和 X-Accel 权限问题。
  修复:除唯一基础组件外，所有运行时图片统一经 `AuthenticatedAssetImage`，受保护素材先用当前会话 Bearer 获取 Blob，再以受控 object URL 渲染；图片下载继续复用同一鉴权 fetch。原生 `<video>/<audio>` 为保留 Range 流式播放不改成整文件 Blob，但所有成功的 MySQL/本地 Bearer API 都会修复作用域仅为 `/api/assets/file/` 的 HttpOnly、SameSite=Strict 媒体 Cookie，避免登录初始化路径漂移再次使原生媒体失去凭证。服务端 owner 校验和匿名 403 保持不变，签名 capability 不持久化。
  验收门禁:`managedAssetRenderBoundary.test.mjs` 递归扫描全部 `src/**/*.tsx`，除基础组件外出现任何新 `<img>` 都直接失败；`managedAssetSessionCookie.test.mjs` 锁定 MySQL/本地两套认证边界都修复媒体 Cookie。提交 `faa81f3` 已通过完整 `npm run verify`（TypeScript/ESLint、144 个服务端测试文件、189 个前端测试文件、23 个脚本测试文件和生产构建）及 Agent Center/OneClick/provider 额外回归。release `meiao-20260729145631-d64f591090f0` 等到活动付费任务归零后由标准 drain/mutex 流程发布，公网与 host-local health、worker、COS 均 healthy；云上四个关键源文件哈希与本地一致。董丹丹账号 `/api/state` 200 并自动补齐 scoped HttpOnly/Secure/Strict Cookie；两张精确 styleRef 分别通过 Bearer 与原生 Cookie GET 200，MIME、176361/401274 bytes 和 SHA-256 均与数据库一致，匿名仍 403。受控浏览器当前是另一个无故障项目的登录态，未冒充董丹丹真实 DOM；最终用户页面观察仍与资源链证据分开记录。
  如何避免:**素材鉴权是全局渲染基础设施，不是某张卡片的局部补丁。安全策略变化后必须用仓库级结构门禁枚举所有图片消费者；图片统一走 Bearer→Blob，需 Range 的原生媒体统一走短路径 HttpOnly Cookie。生产验收仍要同时保留故障账号真实 UI、服务端授权、文件字节和 Nginx 交付证据，任何一层不能替代另一层。**

- **#97 ✅ 已发布并完成真实资源链验收(2026-07-29)· 受保护公共模特图被 direct-first 重新变成 KIE 无权读取的正式域名直链**
  根因:#82 把 `/api/assets/file/...` 收紧为鉴权读取后，公共模特 `internal` 素材没有可交给外部 provider 的公网签名 URL；但 #45 的 `direct-first` 逻辑没有区分“没有解析器”和“解析器已确认只能内部读取”，把结果再次拼成 `https://meiaoyuntai.com/api/assets/file/...`。KIE 已创建 `gpt-image-2` 任务后才异步抓图，三张模特图匿名读取均为 403，因已有 `providerTaskId` 又不能自动转存重提。
  修复:`internal` 素材的 provider 解析器签发只对本机回环有效、绑定 asset ID 与真实 owner 的 HMAC capability；generation/chat 只用该 capability 读取原字节并在付费提交前完成 KIE File Upload，绝不把回环 capability 或需登录的正式域名直链交给外部服务。只有完全没有解析器的调用才允许 `MEIAO_PUBLIC_BASE_URL` 直连兜底。COS 签名读取、非托管公网 URL、已有 task id 禁止重提的安全合同均保持不变。提交 `6e98d57` 通过完整 `npm run verify` 和二次独立复审 Critical/Important/Minor 全 0；release `meiao-20260729165941-6e98d573260d` 发布后，韩系女生 `001 / v2` 的真实 `front_close` 资产经 capability 回环读取为 `200 image/jpeg`、678986 字节且与数据库一致，非付费模拟暂存确认 capability 不会成为 provider 输出，匿名正式域名仍为 403；公网/宿主机 health、worker、COS、源码哈希、权限与前端资源链均通过。未创建付费 canary，也未自动重提已有 task ID 的历史失败任务。
  如何避免:**素材解析必须保留三态：无解析器、外部可读签名 URL、只供服务端读取的回环 capability。任何受保护 `/api/assets/file/...` 都不能因公网 host 被重新判成 provider 直链，任何回环 capability 也不能泄露给外部 provider；鉴权策略变化必须同时回归浏览器消费者、服务端素材 route 和 generation/chat provider 消费者，KIE recordInfo 的最终 `input_urls` 才是 provider 实际收到内容的证据。**
- **#98 ✅ 已修(2026-07-28)· 持久音色试听把目标语言算进身份，同一音色切语言仍重复付费生成**
  根因:持久化试听的 `previewId` 和前端内存缓存都使用“账号 + 目标语言 + 音色”，但产品承诺的可复用资源实际是“当前账号拥有的某个音色样本”；目标语言只决定首次生成样本文案，不改变音色身份。结果是 Zephyr 英文已经永久保存后，中文页面仍创建了第二个 KIE TTS task，界面还把持久缓存查询统一写成“正在生成”，进一步误导用户。
...
- **#108 ✅ 已用云上真实任务回放并部署(2026-08-04)· 产品替换 v5 策划被重复投影，生图提示词超过 provider 文本上限**
  根因:v4/v5 在旧产品描述结构上追加 `identityLock` 和 `colorPreservation`，但生图投影同时序列化完整 `products`、包含颜色子合同的身份合同，以及再次抽取的颜色合同。同一颜色规则出现三次，产品材质/细节/图案/结构出现两次，区域 `generationInstruction` 又重述已分字段的执行关系。多桑两条任务的最终 prompt 分别达 28319/23234 字符，KIE 在创建任务前返回 `The text length cannot exceed the maximum limit`；旧测试只校验合同存在和顺序，没有真实长分析投影与字符上限回归。
  修复:完整策划回复仅作审计产物；生图改为三份不重复的执行投影：颜色合同唯一保留固有色、逐组件颜色、相对关系、中间调和禁止偏移；非颜色身份合同唯一保留材质、细节、图案、结构、Logo/图形拓扑、实体边界、可见文字、视觉锚点、具体不变特征和排除元素；策划区只保留尺度、透视、光影、材质互动、遮挡和接触阴影。结构数据用紧凑 JSON，并新增 `VITE_MEIAO_PRODUCT_REPLACE_GENERATION_PROMPT_MAX_CHARS` 默认 18000 的付费前闸门；provider 即使再返回文本超限也归为不可重试 `provider_bad_request`。真实回放 28319→14407、23234→12147、17194→9059，五维产品事实与产品身份锚点仍完整保留。
  如何避免:**分析模型的完整 JSON 是审计真相，不是生图 payload；每个事实只能有一个权威执行字段，新增合同前必须明确替换哪个旧字段，不得在完整策划上继续叠加。提示词回归必须同时校验字段唯一性、五维完整性、真实长分析级别与 provider 安全上限，超限时只能停止付费提交，禁止截断身份或颜色真值。**

- **#109 ✅ 本地已修、待部署(2026-08-04)· 出海翻译不同失败图片不能并发重试**
  根因:单图重试虽然已有 `regenerate:<projectId>:<resultId>` 结果级独占键，仍叠了三道过粗的串行门禁：`ProjectCard` 发现项目内任一结果生成中就禁用全部重试按钮；`handleRegenerateResult` 按模块活动任务再拦截一次；翻译专用锁又只用 `translation:<subFeature>` 作 key。因此两张完全不同的失败图片，也必须等第一张终态后才能提交第二张，截图中就表现为“当前模块仍有任务生成中”。
  修复:翻译项目不再使用通用项目/模块级重生成冲突锁；翻译专用锁收窄为 `translation:<projectId>:<retryRootResultId>`，同一原始图片及其重试链仍互斥，不同图片可同时提交；前端按钮同步只尊重当前结果的 pending/generating，不再被兄弟结果锁死。其他模块的项目级门禁不变；“批量重试”按钮仍使用 `runTranslationRetriesSequentially`逐个执行，本次只解除多张单图手动重试的不必要串行。
  如何避免:**付费任务的互斥 key 必须使用真正的幂等/扣费身份，不得用“整个项目/模块/子功能”代替。同图防重由 project + retry root 保护，不同图的并发度交给后端账号并发门控；批量操作的节奏与手动单项操作的并发能力要分开建模。**

- **#110 ✅ 已用真实 provider payload 回放并部署(2026-08-04)· 产品/Logo 替换生图提示词仍是多份合同与长固定约束的叠加**
  根因:#108 只解决了“超 provider 上限”，未把“每个事实只有一个权威执行位置”贯彻到字段内部。产品合同中 `invariantDetails`/`forbiddenChanges` 重述颜色、纹理、Logo 和结构，场景策划又重述参考图可直接观察的旧产品、尺度和光线。Logo 生图同时发送分析、绑定、几何三份 JSON，`generationInstruction` 再复述局部信息，14 条英文固定约束与末尾 7 条 guardrails 再重复一次。
  修复:产品改为每个产品一份 `identity + color` 执行合同，仅保留实体边界、可见文字、Logo/图形、视觉锚点、排除元素、材质、细节、图案、结构与结构化颜色；场景只保留位置、透视、材质互动、遮挡和接触阴影。Logo 将映射、目标框、contain 边界、身份排布和表面融合合并为一份合同，移除完整分析投影、重复指令和末尾护栏。图片改用 Image 编号而非 URL，坐标收敛到 4 位小数。用户附件三产品 prompt `14568→8738`；云上真实单区 Logo prompt `6964→1785`。
  如何避免:**提示词验收不能只看 provider 上限；必须统计真实 payload 的分段占比，锁定“一个事实一个字段”、最大字符目标和图片角色编号。精简只删重复语义与可从输入图直接读取的描述，不截断产品或 Logo 身份真值。**

- **#111 ✅ 已用真实附件回放并部署(2026-08-04)· 策划模型不应把已上传的视觉身份再翻译成文本**
  根因:#110 仍假设产品与 Logo 的身份真值必须由策划模型先用文本完整描述，再传给生图模型。但生图请求本来就携带产品/Logo 素材图，自然语言转述不仅数千字符地重复图像，还可能把颜色、结构或图形误读成与真实像素冲突的第二份身份。
  修复:产品策划升为 v6，仅输出每区域放置、透视、材质互作、遮挡与接触阴影；Logo 策划升为 v4，仅校验选框并输出表面、透视、光线、材质与遮挡。单项决策上限 120 字符，当前解析器默认拒绝旧 schema，仅恢复路径显式兼容。生图分别只收到一份“图片编号→数值区域→局部融合”执行合同；Logo 编号标记图也改为只进策划、不进生图。用户三产品附件旧 v5 回放从 14568 降至 3803，当前 v6 三产品典型为 1861，Logo 典型为 1338。commit `48672fa`，release `meiao-20260804143339-48672faa4e17`；发布后精确 release health、Temporal worker 与 COS 真探针通过。
  如何避免:**多模态链路中，上传图像是视觉身份的唯一真值；策划只应输出无法从图像绑定中确定的执行决策。生图提示词应是紧凑执行协议，不是素材图的文字副本；固定约束仅解决映射、保真优先级和禁止行为，不再造身份事实。**

- **#112 ✅ 本地已集成、禁止部署(2026-08-06)· 出海翻译同文映射未经充分复审并锁死批次自然译法**
  根因:目标语言守卫只检查文字体系，合法的 `source === target` 可编辑映射首轮就被接受；批次一致性注册表又把该同文结果登记为标准，导致后续图片更自然的改写被覆盖回原文。
  修复:首次可编辑同文映射使用既有第二次策划额度做一次有界复审，携带首轮完整映射，按当地母语电商编辑是否会直接发布检查词义重复、固定搭配和文案角色；第二次合法结果即使仍相同也接受并继续生图。保护项不触发复审，同文结果不登记为首个批次标准，后续首个非同文目标才能建立一致性。
  如何避免:**同语种文案不能跳过质量检查，但也不得为了制造差异强制改写；同文复审最多一次，保护内容不参与，批次一致性只能由非同文目标建立。**
