...
- **#86 ✅ 本地已修、待发布(2026-07-31)· 稳定托管素材身份未恢复真实媒体路径，口播去文案在 provider 提交前失败**
  ...
  修复:统一的托管素材身份解析器新增严格 `managed://<safe-id>` 提取；读取解析仍必须经过数据库 active、未删除和真实 owner 校验，再从资产记录恢复 `/api/assets/file/.../source.mp4`，签发只供本机读取的 owner-bound capability。provider 暂存只使用恢复后的真实 URL 推导 MP4 文件名和 MIME；稳定身份缺少 owner-aware resolver 或 resolver 返回空时，必须在 fetch/upload 前以 `managed_asset_unavailable` fail closed，不得回落到通用 URL 归一化。稳定身份、回环 capability 或正式域名无凭证直链都不能交给外部 provider。现有 `/api/assets/file/...`、COS 签名 URL、跨账号拒绝和已有 `providerTaskId` 只查询不重提的合同保持不变。
  如何避免:**持久化稳定身份、应用内可读地址和 provider 可消费地址是三种不同合同。任何新增 `managed://` 消费者必须通过统一解析器恢复资产记录，不能把身份串当 URL 交给 FFprobe 或第三方；回归必须同时锁定 active/owner 校验、真实文件名/MIME、capability 不外泄、跨账号拒绝以及“provider 提交前失败时 task ID 仍为空”。**
