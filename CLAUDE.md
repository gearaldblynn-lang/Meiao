...
- **#50 ✅ 已修(2026-07-13)· 发布就绪双检仍有 TOCTOU,检查后新任务可在 PM2 restart 前进入**
  ...
  修复:最终切换先用精确 `iptables` IPv4 规则拒绝新的 Nginx 回源/公网直连,`ip6tables` 规则拒绝 IPv6 直连 3100,等已有连接连续为零;再由持有 `internal_jobs WRITE` 锁的同一连接复查 running=0、停旧 PM2、验证停机和会话仍存活后才回写 ack。网络规则以各自的 command + `-C/-D` 精确、幂等清理,每清一条立即持久化剩余状态。health 失败只在 `pm2 pid` 成功且显式返回全零时认定已停;旧进程已停但新进程未起、无法证明停机或规则清理失败时,门禁保留且 marker 写为永不过期的 `manual`。
  如何避免:**首发兼容不能依赖新代码才识别的 marker。网络隔离必须同时覆盖 IPv4/IPv6,清理要可重试且不能把命令错误当成“规则不存在”;失败清理必须以“可提交进程已健康或已证明停止”为释放门禁的先决条件。**
