...
- **#82 🔄 二次修复已发布，故障账号真实 UI 仍待观察(2026-07-29)· 站内结果已成功落盘，但项目卡查看和下载统一 403**
  根因:公共模特素材加固把 owner/access-key 校验前移到所有 `/api/assets/file` 分支，但普通项目卡仍用原生媒体元素直接请求历史无签名 URL，无法携带 localStorage Bearer。第一次修复增加了素材 Cookie、下载代理和新结果签名 URL，却没有改掉 `ProjectCard` / `PlanEditor` 的原生 `<img>` 消费边界；验收只证明了人工携带 Cookie/签名的素材请求为 200，没有在“多桑”真实项目网格中确认浏览器实际请求，因此项目 2–9 仍持续破图。截图中可见的 `asset_key` 是破图后暴露的 prompt 文本里的输入素材 URL，不是结果图自身地址。
  修复:项目网格、详情大图、翻译版本缩略图和方案编辑器统一改用 `AuthenticatedAssetImage`，对历史和未来的 managed asset 先用当前会话 Bearer 获取 Blob，再以 object URL 交给图片元素；单图/ZIP 下载继续复用同一个鉴权 fetch 边界。撤销第一次修复中把 HMAC `asset_key` 持久化进 buffer、stream 和 COS 资产记录的做法，恢复只存稳定无签名 URL，Cookie 仅保留为原生媒体的兼容兜底。
  验收门禁:新增 `ProjectCard.authenticatedAssetImage.test.mjs`，禁止项目结果重新退回原生 `<img src={result.imageUrl}>`；`assetStore.test.mjs` 明确即使配置访问密钥也不得把 capability 写入 `public_url`。release `meiao-20260729113350-be3b4a1f4714` 已发布，公网入口实际引用 `ShellMigratedApp-HbQtLgIR.js`；“多桑”项目 1–9 的 9 个精确结果资产均验证匿名 403、当前 owner 签名 200、`image/jpeg`、字节数和 SHA-256 与数据库一致，且存储状态 active。以上只能证明资源和授权链已恢复；完成状态仍必须在“多桑”真实登录页面刷新后确认卡片图片均为 `blob:`、`complete=true`、`naturalWidth>0`，并从同一项目执行一次真实下载。当前受控浏览器登录态的首图列表为空，不是该故障视图，所以不得把本轮资源探针虚报成真实 UI 验收。
  如何避免:**素材访问安全必须集中在可复用的浏览器加载边界，业务组件不得自行用原生 URL 猜测认证行为；签名 capability 不得持久化。生产验收必须从故障账号、故障项目和真实 DOM/下载动作出发，同时保留文件、数据库、Node、Nginx 的字节/哈希证据，任何一层替代另一层都不能宣布完成。**
