...
- **#79 🟡 本地已修、待发布验收(2026-07-23)· 单上游先停后启发布必然制造官网 502**
  ...
  修复:正式 PM2 合同改为单实例 `cluster + wait_ready`，完成 bootstrap/listen 后才发 ready，旧进程收到信号后优雅停 worker、Temporal、HTTP 和连接池。发布脚本先用 marker 暂停新写入和 worker，等待活动写请求/任务归零。经典 MySQL worker 与 Temporal activity 在 claim 前必须获取同一 MySQL 命名锁并重查 marker；部署用该锁做最终零任务屏障，从而关闭“worker 先检查、排队等锁、部署释锁后再抢跑 claim”的付费竞态。释放锁后原子切 dist 并 `pm2 startOrReload`。只有精确 release health 通过才移除 marker；若仅旧内存进程健康，因磁盘源码/依赖已属于未验证 release，失败清理必须保留永不过期 `manual` marker 与 mutex，禁止新写入，不能假报完整回滚。旧 fork 首次迁移必须用只监听 `127.0.0.1:3101` 的候选与 Nginx 双切换，且生产代码对 3101 非回环绑定直接拒绝启动；后续才全部走标准脚本。
