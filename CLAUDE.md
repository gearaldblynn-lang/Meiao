...
- **#86 ✅ 本地已修、待发布(2026-07-31)· 稳定托管素材身份未恢复真实媒体路径，口播去文案在 provider 提交前失败**
  ...
  修复:统一的托管素材身份解析器新增严格 `managed://<safe-id>` 提取；任何保留 `managed:` scheme 但带 query/hash、缺 host 或非规范分隔的输入都在 provider 入口拒绝。读取解析必须经过数据库 active、未删除、非空真实 owner 校验，再验证资产行 `publicUrl` 确实为同一 asset ID 的 `/api/assets/file/...` 路径，最后才签发只供本机读取的 owner-bound capability。provider 暂存只使用恢复后的真实 URL 推导 MP4 文件名和 MIME；稳定身份缺少 owner-aware resolver、resolver 返回空或资产行路径失配时，必须在 fetch/upload 前以 `managed_asset_unavailable` fail closed，不得回落到通用 URL 归一化。稳定身份、回环 capability 或正式域名无凭证直链都不能交给外部 provider。现有 `/api/assets/file/...`、COS 签名 URL、明确 allowlist 的共享虚拟模特、跨账号拒绝和已有 `providerTaskId` 只查询不重提的合同保持不变。
  如何避免:**持久化稳定身份、应用内可读地址和 provider 可消费地址是三种不同合同。任何新增 `managed://` 消费者必须通过统一解析器恢复资产记录，不能把身份串当 URL 交给 FFprobe 或第三方；回归必须同时锁定规范 scheme、active/owner、资产行路径与 ID 一致、真实文件名/MIME、capability 不外泄、跨账号拒绝，以及所有拒绝分支 fetch/upload/HEAD/sign 均为 0。**
