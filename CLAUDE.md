...
- **#83 ✅ 本地已修、待部署(2026-07-29)· 受保护公共模特图被 direct-first 重新变成 KIE 无权读取的正式域名直链**
  ...
  修复:`internal` 素材的 provider 解析器签发只对本机回环有效、绑定 asset ID 与真实 owner 的 HMAC capability；generation/chat 只用该 capability 读取原字节并在付费提交前完成 KIE File Upload，绝不把回环 capability 或需登录的正式域名直链交给外部服务。只有完全没有解析器的调用才允许 `MEIAO_PUBLIC_BASE_URL` 直连兜底。COS 签名读取、非托管公网 URL、已有 task id 禁止重提的安全合同均保持不变。
