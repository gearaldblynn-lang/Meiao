...
- **#82 🔄 二次修复待云上真实 UI 验收(2026-07-29)· 站内结果已成功落盘，但项目卡查看和下载统一 403**
  ...
  验收门禁:新增 `ProjectCard.authenticatedAssetImage.test.mjs`，禁止项目结果重新退回原生 `<img src={result.imageUrl}>`；`assetStore.test.mjs` 明确即使配置访问密钥也不得把 capability 写入 `public_url`。发布完成前必须在“多桑”真实登录页面刷新后确认项目 2–9 的卡片图片均为 `blob:`、`complete=true`、`naturalWidth>0`，并从同一项目执行一次真实下载；人工构造的 Curl/Cookie/签名 200 不能再代替 UI 验收。
